-- v13.509.0 · Resolución tolerante de proveedor por nombre: exacto y, si no
-- hay, coincidencia única por prefijo (evita ambigüedades silenciosas).
...
-- Backfill 2/2: cotizaciones con proveedor único → se repone en los costos
-- vacíos del embarque aunque el nombre del concepto no coincida literal.
WITH unico AS (
...
-- Resolución del id con la función tolerante.
UPDATE public.conceptos_costo cc
