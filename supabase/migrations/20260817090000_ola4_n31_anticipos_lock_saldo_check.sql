-- Ola 4 · N31: aplicar_anticipo_a_factura leía anticipos_proveedor sin lock
-- y validaba saldo_disponible sobre esa lectura; dos transacciones
-- concurrentes podían aplicar el mismo anticipo por montos que sumados
-- exceden el saldo, dejando saldo_disponible NEGATIVO tras el trigger de
-- recálculo. Fix en dos capas:
--   1) SELECT ... FOR UPDATE en aplicar_anticipo_a_factura y en
--      cancelar_anticipo_proveedor (cierra también la carrera
--      cancelar-vs-aplicar) para serializar el acceso al anticipo.
--   2) CHECK (saldo_disponible >= 0) NOT VALID como defensa final a nivel
--      BD; se valida aparte tras sanear históricos si los hubiera.
-- Base: 20260810130000_ola4_n12_anticipos_guard_org (íntegra); sólo se
-- agrega el FOR UPDATE a los SELECT existentes.

-- ============================================================
-- PASO PREVIO (manual, ejecutar y revisar ANTES de VALIDATE CONSTRAINT):
--
--   SELECT id, monto, saldo_disponible, estado
--     FROM public.anticipos_proveedor
--    WHERE saldo_disponible < 0;
--
-- Si devuelve filas, sanearlas (recalcular desde anticipos_aplicaciones)
-- antes de ejecutar el VALIDATE CONSTRAINT comentado al final.
-- ============================================================
...
-- Ola 4 · N31: defensa final a nivel BD. NOT VALID para no romper la
-- migración si ya existieran saldos negativos históricos.
ALTER TABLE public.anticipos_proveedor
  DROP CONSTRAINT IF EXISTS anticipos_proveedor_saldo_no_negativo;
ALTER TABLE public.anticipos_proveedor
  ADD CONSTRAINT anticipos_proveedor_saldo_no_negativo
  CHECK (saldo_disponible >= 0) NOT VALID;

-- Ejecutar manualmente tras sanear históricos (ver PASO PREVIO arriba):
-- ALTER TABLE public.anticipos_proveedor VALIDATE CONSTRAINT anticipos_proveedor_saldo_no_negativo;
