CREATE INDEX IF NOT EXISTS idx_efe_embarque_estado
  ON public.embarque_facturas_entrantes (embarque_id, estado)
...
BEGIN
  ...
  v_ok := (v_ent_pendientes=0); v_puede := v_puede AND v_ok;
  v_checks := v_checks || jsonb_build_array(jsonb_build_object(
  ...
  SELECT COUNT(*), COALESCE(array_agg(nombre ORDER BY nombre), ARRAY[]::text[])
    ...
    FROM (
      SELECT DISTINCT cc.proveedor_id,
             COALESCE(NULLIF(cc.proveedor_nombre,''), 'Proveedor sin nombre') AS nombre
