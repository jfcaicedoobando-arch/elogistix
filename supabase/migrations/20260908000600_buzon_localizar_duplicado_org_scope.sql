...
--
-- v13.821.7 (OLA P2-1): defensa en profundidad contra una relación corrupta
-- entre tablas (p. ej. `embarque_facturas_entrantes.proveedor_factura_id`
-- apuntando a una factura de OTRA organización, o `proveedor_facturas.embarque_id`
-- apuntando a un embarque de otra organización). Antes esos casos podían filtrar
-- `factura_id`/`embarque_id`/folio si el documento raíz era visible: ahora se
-- revalida la organización en CADA salto (documento → factura → embarque) y
-- cualquier inconsistencia cross-org colapsa a 'ajeno' sin ids.
--
-- Casos:
...
--   'ajeno'           → duplicado no visible para el llamante (o relación
--                        cross-org inconsistente): genérico, sin ids.
