-- Refactor auditoria_embarques_org (AUD-2):
--   1. Unifica la matriz de documentos exigidos usando la función canónica
--      public._docs_requeridos_por_estado(modo, estado) en vez de una copia
--      hardcodeada, eliminando el falso positivo/negativo por divergencia.
--   2. Excluye embarques soft-deleted (deleted_at IS NULL) de todas las reglas.
--   3. docs_pendientes_avanzado ahora sólo cuenta pendientes de documentos
--      que efectivamente aplican al modo/estado del embarque.
--   4. Sustituye el fallback silencioso de tipo de cambio (17.5/19.0) por un
--      hallazgo explícito `tipo_cambio_faltante` (severidad medio), y la
--      regla de margen ignora embarques sin TC en moneda extranjera.
