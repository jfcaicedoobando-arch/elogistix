...
-- Categoría y subtipo de gasto operativo para proveedores
DO $$ BEGIN
  CREATE TYPE public.categoria_proveedor AS ENUM ('Logistico', 'GastoOperativo');
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

DO $$ BEGIN
  CREATE TYPE public.subtipo_gasto_operativo AS ENUM (
    'Renta','Servicios','Papeleria','Software','Honorarios',
    'Mantenimiento','Marketing','Viaticos','Otros'
  );
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

ALTER TABLE public.proveedores
  ADD COLUMN IF NOT EXISTS categoria public.categoria_proveedor NOT NULL DEFAULT 'Logistico',
  ADD COLUMN IF NOT EXISTS subtipo_gasto public.subtipo_gasto_operativo NULL;

-- Permitir tipo NULL cuando categoría = GastoOperativo
ALTER TABLE public.proveedores ALTER COLUMN tipo DROP NOT NULL;

-- Integridad: si Logístico → requiere tipo; si GastoOperativo → requiere subtipo_gasto
ALTER TABLE public.proveedores DROP CONSTRAINT IF EXISTS proveedores_categoria_check;
ALTER TABLE public.proveedores ADD CONSTRAINT proveedores_categoria_check CHECK (
  (categoria = 'Logistico' AND tipo IS NOT NULL)
  OR (categoria = 'GastoOperativo' AND subtipo_gasto IS NOT NULL)
);

-- RPC ampliado: búsqueda global + filtros categoría/subtipo + tipo opcional
CREATE OR REPLACE FUNCTION public.proveedores_listado(
  ...
  p_subtipo_gasto text DEFAULT NULL::text
)
RETURNS TABLE(
  id uuid, nombre text, tipo public.tipo_proveedor, rfc text, contacto text,
  moneda_preferida public.moneda, pais text, origen_proveedor text,
  categoria public.categoria_proveedor, subtipo_gasto public.subtipo_gasto_operativo,
  total_operaciones bigint, monto_pendiente numeric, total_count bigint
)
LANGUAGE sql
STABLE
