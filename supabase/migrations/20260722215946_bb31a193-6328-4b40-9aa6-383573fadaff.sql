-- v13.308.3 — Fix: recalcular_estado_factura debe levantar el flag
-- app.recalc_estado_factura='1' durante su propio UPDATE, para que el
-- guard_estado_factura permita la transición a Pagada / Parcialmente
-- pagada / Vencida disparada por el recálculo automático.
--
-- Sin este fix, cualquier INSERT en pagos_factura hecho por un usuario
-- que no sea super_admin dispara el trigger de recálculo, éste intenta
-- fijar el estado calculado, y el guard lo bloquea con
-- LC_FAC_ESTADO_CALCULADO. Detectado en RLS suites financiero_critico
-- y roles_no_admin.
