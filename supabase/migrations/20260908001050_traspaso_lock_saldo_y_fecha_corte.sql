-- Ola 8 · Corrección P1 sobre registrar_traspaso_bancario (tesorería).
--
-- Defecto 2 (concurrencia): el saldo de la cuenta origen se leía sin
-- bloquear las filas de cuentas_bancarias, así que dos traspasos
-- simultáneos podían leer el mismo saldo disponible y ambos pasar la
-- validación, sobregirando la cuenta. Se agrega un
-- `SELECT id ... FOR UPDATE` sobre origen y destino, ordenado por id
-- ascendente (evita deadlocks entre traspasos cruzados), y el saldo se
-- revalida DESPUÉS de tomar el candado.
--
-- Defecto 3 (fecha anterior al corte): la función aceptaba p_fecha
-- anterior a fecha_saldo_inicial de cualquiera de las dos cuentas. Como
-- saldo_cuenta_bancaria() sólo suma movimientos con fecha >=
-- fecha_saldo_inicial, el movimiento quedaba "Conciliado" sin afectar el
-- saldo (o afectando sólo una pierna si las cuentas tienen cortes
-- distintos). Se rechaza con LC_TRASPASO_FECHA_ANTERIOR_CORTE indicando
-- la fecha mínima permitida.
--
-- Cuerpo vigente tomado de supabase/migrations/20260905000200_ola8_v15_candados.sql
-- (B-6), con los dos candados agregados; el resto del cuerpo no cambia.
