...
BEGIN
  ...
  RETURN QUERY
  WITH candidatos AS (
    -- Match directo por nombre en campos de texto del embarque
    SELECT
      e.id AS embarque_id,
      e.expediente,
      e.cliente_nombre,
    ...
    -- Match vía tarifa aplicada → agente.proveedor_id
    SELECT
      e.id, e.expediente, e.cliente_nombre, e.estado, e.etd, e.eta,
    ...
    -- Match vía tarifa aplicada → naviera condiciones
    SELECT
      e.id, e.expediente, e.cliente_nombre, e.estado, e.etd, e.eta,
