...
BEGIN
  ...
  -- Fase 2B: Cobranza (CXC) — factura timbrada vencida sin pago total
  facturas_saldo AS (
    SELECT f.id,
           f.total - COALESCE((
  ...
  -- Fase 2B: CXP — facturas de proveedor en captura estancadas
  hall_cxp_captura AS (
    SELECT jsonb_build_object(
      'embarque_id', e.id, 'expediente', e.expediente,
      'cliente_nombre', e.cliente_nombre, 'modo', e.modo::text, 'estado', e.estado::text, 'eta', e.eta,
  ...
  -- Fase 2B: CXP — facturas de proveedor vencidas y vigentes (sin pago)
  hall_cxp_vencida AS (
    SELECT jsonb_build_object(
      'embarque_id', e.id, 'expediente', e.expediente,
      'cliente_nombre', e.cliente_nombre, 'modo', e.modo::text, 'estado', e.estado::text, 'eta', e.eta,
