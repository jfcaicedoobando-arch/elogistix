-- Ola 5 · RG4-2: reaplicar N41 (valuación canon de gastos operativos) y N45
--   (excluir 'Sustituida' del flag facturado) ENCIMA de la definición vigente
--   de 20260811090000_ola4_n10 (que es la que conserva el guard Borrador
--   B-033, FIX C5, org_scope() y 'Por liquidar').
-- Historia: 20260811090000 ordena después de la reaplicación 20260810203939
--   y sobrescribió los dos fixes con el código pre-fix (también en
--   producción), dejando N41/N45 sin efecto real.
-- Base EXACTA: 20260811090000. ÚNICOS cambios:
--   (a) N41 en gastos_op_facturas de dashboard_summary (+ CTE aditivo
--       gastos_op_sin_tc, llave aditiva gastosOperativosSinTC y columna
--       tipo_cambio_eur en embarques_base);
--   (b) N45 en mes_sig_src de dashboard_details.
-- RETURNS jsonb sin cambios en ambas funciones (sin riesgo 42P13).
  ...
  RETURN (
    WITH embarques_base AS (
      SELECT e.id, e.estado::text, e.modo, e.tipo, e.etd, e.eta,
        -- Ola 5 · RG4-2 (N41): TC EUR del embarque, requerido por
        -- gastos_op_facturas para valuar facturas de gasto en EUR
        -- (proveedor_facturas no tiene columna de TC EUR).
        e.tipo_cambio_eur,
        CASE
    ...
    -- Ola 5 · RG4-2 (N41): valuación por moneda propia del gasto — MXN
    -- directo; USD sólo con tipo_cambio_usd > 1 (la columna es NOT NULL
    -- DEFAULT 0, así que el viejo IS NOT NULL siempre era cierto y el USD
    -- sin TC aportaba total*0 = 0 sin avisar); EUR con el tipo_cambio_eur
    -- del EMBARQUE vinculado (antes se valuaba erróneamente con el TC USD).
    -- Sin TC confiable: no suma.
    ...
    -- Ola 5 · RG4-2 (N41): conteo de facturas de gasto excluidas por falta
    -- de TC confiable (llave aditiva gastosOperativosSinTC, no rompe
    -- parsers existentes).
    gastos_op_sin_tc AS (
      SELECT COUNT(*) AS val
      FROM proveedor_facturas pf
      ...
      WHERE pc.tipo_contable IN ('Venta','Administracion')
        ...
        AND NOT (pf.moneda = 'EUR' AND eb.tipo_cambio_eur > 1)
    ),
