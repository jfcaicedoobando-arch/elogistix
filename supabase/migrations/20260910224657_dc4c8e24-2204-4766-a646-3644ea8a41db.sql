...
BEGIN
  ...
  IF COALESCE(v_mov.hash_dedupe,'') LIKE 'cobro-%' THEN
    -- Movimiento espejo generado por el sistema: el dinero nunca entró.
  ...
  ELSE
    -- Movimiento real del estado de cuenta: el dinero sí entró; sólo se libera.
    UPDATE bbva_movimientos
...
CREATE OR REPLACE FUNCTION public._trg_reversar_movimiento_rep_cancelado()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
...
DROP TRIGGER IF EXISTS trg_reversar_movimiento_rep_cancelado ON public.pagos_factura;
CREATE TRIGGER trg_reversar_movimiento_rep_cancelado
AFTER UPDATE OF estado_rep ON public.pagos_factura
FOR EACH ROW
EXECUTE FUNCTION public._trg_reversar_movimiento_rep_cancelado();