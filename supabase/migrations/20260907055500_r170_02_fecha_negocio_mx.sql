-- R170-02 · Fecha de negocio (hora México) dentro de la MISMA transacción.
--
-- Antes: `crear_proforma_atomica` dejaba `fecha_emision` con el DEFAULT
-- CURRENT_DATE (UTC) y `convertir_proformas_a_factura` insertaba los borradores
-- con CURRENT_DATE / CURRENT_DATE + credito. Entre las 18:00 y 23:59 CDMX eso
-- ya es el día siguiente en México, y el cliente lo "reparaba" después con un
-- UPDATE best-effort que podía fallar en silencio (y que se retira en este
-- mismo cambio).
--
-- Ahora la fecha se calcula en el servidor, en la transacción que crea el
-- documento: `(now() AT TIME ZONE 'America/Mexico_City')::date`. El vencimiento
-- se deriva de esa misma fecha, por lo que los días de crédito se conservan
-- (crédito 0 vence el mismo día; crédito > 0 mantiene su plazo).
--
