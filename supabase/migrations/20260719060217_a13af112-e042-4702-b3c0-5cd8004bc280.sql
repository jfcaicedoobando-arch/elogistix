-- Fase R.6 (Bug 18): guarda BD para revalidación de tarifa antes de crear borrador de embarque.
...
BEGIN
  ...
  IF v_sev IS DISTINCT FROM 'sin_cambios' THEN
    RAISE EXCEPTION 'LC_TARIFA_REQUIERE_REVALIDACION severidad=% max_delta_pct=%',
...
-- Reemplazo del 1-arg RPC: se añade PERFORM enforce_revalidacion_sin_cambios(...) justo antes
-- de crear el embarque. El resto del cuerpo es idéntico al original.
...
BEGIN
  ...
  -- Fase R.6 (Bug 18): revalidación de tarifa obligatoria.
  PERFORM public.enforce_revalidacion_sin_cambios(v_cot.id);
