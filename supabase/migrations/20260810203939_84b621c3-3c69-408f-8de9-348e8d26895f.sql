-- Ola 4 · N41/N45 (reaplicación): la migración 20260812090000 abortó completa
--   por el cambio de firma de cartera_pendiente, dejando sin efecto estos dos
--   fixes de valuación. Se reaplican aquí sin tocar firmas.
...
BEGIN
  RETURN (
    WITH embarques_base AS (
      SELECT e.id, e.estado::text, e.modo, e.tipo, e.etd, e.eta, e.tipo_cambio_eur,
        CASE
    ...
    -- Ola 4 · N41: valuación por moneda propia del gasto — MXN directo; USD
    -- sólo con tipo_cambio_usd > 1; EUR con el tipo_cambio_eur del EMBARQUE
    -- vinculado. Sin TC confiable: no suma.
      ...
      SELECT eb.*, p.venta_usd, p.costo_usd, p.venta_mxn, p.costo_mxn,
             ...
             EXISTS (
               ...
               WHERE f.embarque_id = eb.id
                 ...
                 -- Ola 4 · N45: 'Sustituida' ya no es CFDI vigente.
