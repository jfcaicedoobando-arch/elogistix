-- R221 (ronda de auditoría · 10 correcciones mínimas)
-- Re-emite el cuerpo canónico de los espejos:
--   1) Cancelado se preserva antes de derivar estado por ETD/ETA
--      (dashboard_details_datos, dashboard_summary_datos, operaciones_stats).
--   2) get_embarque_full devuelve NULL para embarques en la papelera.
--   5) operaciones_stats cuenta TEU reales de embarque_contenedores
--      (40'/45' = 2 TEU) y expone totalContenedoresFisicos.
-- Sin cambios de firma, permisos, RLS ni datos.
...
-- Fuente canónica de public.operaciones_stats
-- Regenerada desde DB (2026-07-23). Cada cambio DEBE actualizarse aquí en el
-- mismo PR que la migración correspondiente. Ver supabase/schema/README.md.
--
-- NOTA (Bloque 3.2 · god-function pendiente): esta función tiene 307 líneas
-- pero NO se dividió en helpers. Razón documentada:
--
--   La función es una cadena monolítica de CTEs (`base` → `profit` →
--   `enriched` → 7 agregadores → `jsonb_build_object` final) donde los 7
--   agregadores dependen de `enriched`. Extraer cualquiera de ellos a una
--   función privada obliga a: (a) recomputar `enriched` dentro de cada
--   helper (regresión de perf 7×), o (b) pasar `enriched` vía TEMP TABLE
--   (rompe `STABLE`, cambia semántica de `SECURITY DEFINER`). Ninguna es
--   refactor puro byte-idéntico — regla de oro #6 del plan.
--
--   La forma correcta de dividirla NO es en funciones plpgsql sino en:
--   (1) una vista materializada `mv_operaciones_enriched` refrescada por
--   trigger, y (2) funciones puras que la consumen. Ese cambio ES un
--   cambio de arquitectura (nuevo objeto, política de refresh, RLS de la
--   MV), no un refactor puro, y debe planearse como ítem separado.
--
-- Estado: canónica establecida. Split queda diferido hasta que se decida
-- introducir la MV o se apruebe cambio de contrato.
