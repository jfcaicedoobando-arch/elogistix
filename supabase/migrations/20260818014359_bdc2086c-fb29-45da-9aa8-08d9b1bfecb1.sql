-- Re-emisión con timestamp final: la versión previa del candado vivía en una
-- migración con fecha anterior a otras ya presentes, así que en una base limpia
-- el orden podía dejar activa la variante que sólo miraba `user_roles`.
...
BEGIN
  ...
  -- El rol operativo vive en organization_members (rol_efectivo); user_roles es
  -- el fallback para roles de plataforma. Sólo mirar has_role bloqueaba a
  -- admin_org legítimos.
