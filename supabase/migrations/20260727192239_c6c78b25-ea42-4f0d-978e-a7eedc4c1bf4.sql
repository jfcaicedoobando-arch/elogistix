-- v13.320.13: hotfix regresión de v13.320.12.
-- El cuerpo de auditoria_embarques_org(uuid) fue re-emitido con la versión canónica
-- del schema, que usa p.estado_proforma (la columna real) en vez de p.estado.
...
BEGIN
  ...
  cxc_facturas_vencidas AS (
    SELECT f.embarque_id, COUNT(*) AS n, SUM(f.total - COALESCE(f.pagado, 0)) AS pendiente
    FROM facturas f
  ...
  cxp_vencidas AS (
    SELECT pf.embarque_id, COUNT(*) AS n,
           SUM(pf.total - COALESCE(pf.pagado, 0)) AS pendiente
    FROM proveedor_facturas pf
    WHERE pf.embarque_id IN (SELECT id FROM emb)
      AND pf.deleted_at IS NULL
