-- v2: convertir proforma(s) a factura(s) con split por moneda (MXN + USD)
-- - Retorna SETOF facturas (0..2 filas).
-- - Conceptos: si la proforma está consolidada → proforma_conceptos_consolidados;
--   si no → conceptos_venta.
-- - Idempotencia: cache almacena {"factura_ids":[...]}; en replay retorna esas filas.
...
BEGIN
  -- Idempotencia: si ya se procesó, retornar las facturas cacheadas.
  ...
  IF v_cached IS NOT NULL AND (v_cached ? 'factura_ids') THEN
    RETURN QUERY
      ...
      WHERE id = ANY(
        ARRAY(SELECT jsonb_array_elements_text(v_cached->'factura_ids'))::uuid[]
      )
  ...
  IF v_total_mxn <= 0 AND v_total_usd <= 0 THEN
  ...
  IF v_total_mxn > 0 THEN
    ...
    ELSE
      ...
      SELECT v_factura_mxn_id, cv.descripcion, cv.cantidad, cv.precio_unitario,
             cv.moneda, cv.cantidad * cv.precio_unitario, v_org, '78101800'
  ...
  -- Marca proformas como facturadas y enlaza IDs (MXN primario, USD secundario).
  UPDATE public.proformas
     SET factura_id           = COALESCE(v_factura_mxn_id, v_factura_usd_id),
         factura_secundaria_id = CASE
           WHEN v_factura_mxn_id IS NOT NULL AND v_factura_usd_id IS NOT NULL
             THEN v_factura_usd_id
           ELSE NULL
         END,
  ...
  -- Idempotencia: guardar IDs para replay.
  ...
  RETURN QUERY
    SELECT * FROM public.facturas
    WHERE id = ANY(v_factura_ids)
    ORDER BY moneda;  -- MXN antes que USD por orden alfabético del enum
END $function$;
