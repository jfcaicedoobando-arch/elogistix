...
BEGIN
  ...
  fact AS (
    ...
    WHERE pf.organization_id = v_oid
  ...
  pag AS (
    SELECT pfc.concepto_costo_id,
           SUM(pp.monto) FILTER (WHERE pp.deleted_at IS NULL) AS pagado_factura
    FROM public.proveedor_facturas_conceptos pfc
    JOIN public.proveedor_facturas pf ON pf.id = pfc.proveedor_factura_id
    LEFT JOIN public.pagos_proveedor pp ON pp.proveedor_factura_id = pf.id
    WHERE pf.organization_id = v_oid
      AND pf.deleted_at IS NULL
      AND pf.estado <> 'Cancelada'
    GROUP BY pfc.concepto_costo_id
  ...
  FROM (
    SELECT cc.id AS concepto_costo_id,
           ...
           COALESCE(p.pagado_factura, 0) AS pagado,
           GREATEST(cc.monto - COALESCE(f.monto_facturado,0), 0) AS por_facturar,
           CASE
