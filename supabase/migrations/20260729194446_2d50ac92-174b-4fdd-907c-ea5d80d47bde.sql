CREATE OR REPLACE FUNCTION public.reabrir_embarque(p_embarque_id uuid, p_usuario_email text, p_request_id uuid DEFAULT NULL::uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Sólo admin o super_admin de la organización pueden reabrir
  ...
  -- Abrir temporalmente el candado de embarque cerrado dentro de esta transacción
  ...
  UPDATE embarques
     SET estado = 'Entregado'::estado_embarque,
         ...
         reabierto_motivo = COALESCE(NULLIF(trim(p_usuario_email), ''), 'reapertura administrativa'),
         updated_at = now()
   WHERE id = p_embarque_id;
  ...
  VALUES (p_embarque_id, 'Embarque reabierto desde Cerrado a Entregado', 'cambio_estado'::tipo_nota, p_usuario_email, v_org_id);
  ...
  BEGIN
    ...
    VALUES (p_embarque_id, v_org_id, 'reabrir', auth.uid(), 'Reapertura desde encabezado del embarque', NULL);
...
REVOKE ALL ON FUNCTION public.reabrir_embarque(uuid, text, uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.reabrir_embarque(uuid, text, uuid) TO authenticated;

-- Alinear permisos de la versión con motivo (usada en la pestaña Cierre)
REVOKE ALL ON FUNCTION public.reabrir_embarque(uuid, text) FROM PUBLIC, anon;
