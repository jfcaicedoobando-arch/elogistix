-- v13.823.280 · Reaplicación de los candados de proformas DESPUÉS de R170-02.
--
-- Problema detectado por el replay ordenado de CI (rls-tests): las migraciones
-- 20260910054026 (LC_OVERRIDE_FUERA_DE_SELECCION en crear_proforma_atomica) y
-- 20260910064636 (LC_PROFORMA_REQUIERE_ACEPTACION en
-- convertir_proformas_a_factura) tienen timestamp ANTERIOR a
-- 20260913000400_r170_02_fecha_negocio_mx.sql, que redefine ambas funciones sin
-- los candados. En producción se aplicaron al final (los candados están vivos),
-- pero un replay limpio los perdía y las suites SQL fallaban.
--
-- Esta migración vuelve a aplicar EXACTAMENTE las mismas definiciones (mismo
-- cuerpo R170-02 + candados), con timestamp posterior a todas las migraciones
-- del corte. Idempotente: CREATE OR REPLACE, sin cambios de datos, permisos ni
-- fórmulas fiscales.
