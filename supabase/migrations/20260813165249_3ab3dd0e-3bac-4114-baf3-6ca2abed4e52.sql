-- Ola 12 · Sprint 04 · R3FE-04
-- proveedor_estado_cuenta_movimientos devolvía TODOS los movimientos del
-- periodo en un solo jsonb (paginación sólo en cliente). Se agregan
-- p_limite/p_offset con defaults, metadata de truncamiento y se preserva el
-- cálculo de saldos sobre el periodo COMPLETO.
...
BEGIN
  ...
  FROM (
    ...
    FROM jsonb_array_elements(v_movs_full) m
