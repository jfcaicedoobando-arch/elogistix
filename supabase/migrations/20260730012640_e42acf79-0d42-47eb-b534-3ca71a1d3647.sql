ALTER TABLE public.cotizaciones
  ADD COLUMN IF NOT EXISTS origen_portal boolean NOT NULL DEFAULT false;

COMMENT ON COLUMN public.cotizaciones.origen_portal IS
  'Q-01b: true cuando la cotización nació de una solicitud del portal del cliente.';

-- Backfill de las solicitudes históricas (marcadas por el prefijo en notas).
UPDATE public.cotizaciones
   SET origen_portal = true
 WHERE origen_portal = false
   AND notas LIKE '[Solicitud desde portal del cliente]%';
