-- Lote financiero M1-M5 (v13.823.384) · parte 1/4
...
-- M4: rastro auditable de cada porción de comisión recuperada.
CREATE TABLE IF NOT EXISTS public.comisiones_recuperaciones (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  organization_id uuid NOT NULL REFERENCES public.organizations(id) ON DELETE RESTRICT,
  liquidacion_id uuid NOT NULL REFERENCES public.liquidaciones_comision(id) ON DELETE RESTRICT,
  comision_id uuid NOT NULL REFERENCES public.comisiones_devengadas(id) ON DELETE RESTRICT,
  ...
  revertida_por uuid,
  CONSTRAINT comisiones_recuperaciones_liq_comision_key UNIQUE (liquidacion_id, comision_id)
);

CREATE INDEX IF NOT EXISTS idx_comisiones_recuperaciones_comision
  ON public.comisiones_recuperaciones (comision_id) WHERE revertida_at IS NULL;
CREATE INDEX IF NOT EXISTS idx_comisiones_recuperaciones_liq
  ON public.comisiones_recuperaciones (liquidacion_id);

GRANT SELECT ON public.comisiones_recuperaciones TO authenticated;
GRANT ALL ON public.comisiones_recuperaciones TO service_role;

ALTER TABLE public.comisiones_recuperaciones ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "recuperaciones_select_org" ON public.comisiones_recuperaciones;
CREATE POLICY "recuperaciones_select_org" ON public.comisiones_recuperaciones
  FOR SELECT TO authenticated
  USING (organization_id = public.current_user_org_id()
         OR public.has_role(auth.uid(), 'super_admin'::public.app_role));

DROP TRIGGER IF EXISTS trg_comisiones_recuperaciones_updated_at ON public.comisiones_recuperaciones;
CREATE TRIGGER trg_comisiones_recuperaciones_updated_at
  BEFORE UPDATE ON public.comisiones_recuperaciones
