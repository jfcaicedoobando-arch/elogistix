-- v13.348.0 — Feed unificado de actividad del embarque.
CREATE INDEX IF NOT EXISTS idx_factura_envios_factura ON public.factura_envios (factura_id);
CREATE INDEX IF NOT EXISTS idx_gar_hist_garantia ON public.embarque_garantias_historial (garantia_id);
CREATE INDEX IF NOT EXISTS idx_bitacora_entidad ON public.bitacora_actividad (entidad_id);
...
BEGIN
  ...
  feed AS (
    ...
    -- 7. Proformas
    UNION ALL
    SELECT ('prof-' || pr.id), 'finanzas', 'proforma', pr.created_at, '',
    ...
    SELECT ('prof-env-' || pr.id), 'finanzas', 'proforma', pr.enviada_at,
           COALESCE(pr.enviada_por, ''), 'Proforma enviada',
           'Proforma ' || COALESCE(pr.numero, '') || ' enviada al cliente', NULL,
           NULL, NULL, 'proforma', pr.id, NULL, NULL
    FROM prof_emb pr WHERE pr.enviada_at IS NOT NULL
    ...
    SELECT ('fac-tim-' || f.id), 'finanzas', 'factura', f.timbrado_en,
           COALESCE(f.timbrado_por, ''), 'Factura timbrada',
    ...
    FROM facturas_emb f WHERE f.timbrado_en IS NOT NULL
  ...
  SELECT * FROM feed WHERE feed.fecha IS NOT NULL ORDER BY 4 DESC;
END;
$$;
