...
BEGIN
  ...
  -- Precondición: la consulta en vivo debe haber devuelto cancellation_status vacío.
  ...
  -- Validar rol admin/facturacion.admin en la org de la factura.
  IF NOT (
    public.has_role(v_user, 'admin')
    OR public.has_role(v_user, 'facturacion_admin')
...
REVOKE EXECUTE ON FUNCTION public.limpiar_cancellation_status_verificado(uuid, text) FROM anon;
