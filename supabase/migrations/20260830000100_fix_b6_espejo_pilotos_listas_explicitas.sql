-- Espejo de replay (FIX B-6): re-emite el fix con timestamp POSTERIOR a
-- 20260827080020_ola8_replay_pilotos_rol_por_org.sql para que un replay en base
-- limpia no reintroduzca has_any_role_in_org (con expansión de jerarquía) en los
-- 3 pilotos financieros. Contenido idéntico a 20260823230747 (aplicada en vivo).

-- =============================================================================
-- FIX B-6 · Pilotos rol-por-org: listas EXPLÍCITAS (sin expansión de jerarquía)
-- =============================================================================
-- Hallazgo H1 de la revisión 2026-08-27 (review_ola678_crm_roles.md): el piloto
-- de la Ola 8 (20260821153354 + replay 20260827080020) llamaba
-- has_any_role_in_org(..., ARRAY['admin','admin_org','contador','tesorero'], …),
-- y ese helper expande cada rol con roles_jerarquia. Como
-- roles_jerarquia('contador') = {contador, auxiliar_contable, admin_org,
-- super_admin}, una membresía `auxiliar_contable` pasaba el chequeo pese a NO
-- estar en las listas explícitas previas al piloto:
--   · registrar_pago_proveedor_lote / registrar_pago_cliente_lote:
--       user_roles ANY {admin, admin_org, super_admin, contador, tesorero}
--   · eliminar_pago_proveedor:
--       es_escritor_financiero = {super_admin, admin, admin_org, contador,
--                                 tesorero, ejecutivo_cobranza}
--
-- Fix conservador: re-emite los 3 cuerpos con el modo EXACTO del helper
-- (has_any_role_in_org_exact, definido abajo) y la lista previa literal.
-- Resultado: el acceso queda EXACTAMENTE como antes del piloto, ahora con
-- scoping por organización (la membresía debe ser de la org del documento).
--
-- DECISIÓN CONSCIENTE: `auxiliar_contable` queda FUERA. Si el negocio quiere
-- que un auxiliar contable opere pagos/cobros en lote o elimine pagos, es un
-- cambio deliberado aparte (añadir el rol al array + test + CHANGELOG), no un
-- efecto colateral de la jerarquía.
--
-- `super_admin` se conserva en los arrays por fidelidad a la lista previa,
-- aunque es rol de plataforma (prohibido en organization_members por
-- _bloquear_rol_plataforma_om): lo cubre el bypass del helper vía has_role.
--
-- Riesgo legacy (H2): usuarios con rol financiero sólo en user_roles y sin
-- membresía equivalente quedan bloqueados. Verificación pre-deploy:
-- scripts/db/predeploy_b6_roles_legacy.sql (riesgo RN-5 en
-- docs/riesgos-aceptados.md).
-- =============================================================================

-- Modo exacto de has_any_role_in_org: misma lectura de membresía y mismo
-- bypass de plataforma super_admin, pero SIN expandir roles_jerarquia.
-- Úsalo sólo cuando la lista de roles autorizados sea una lista cerrada
-- (p.ej. paridad con un chequeo legacy); el modo general con jerarquía sigue
-- siendo has_any_role_in_org.
CREATE OR REPLACE FUNCTION public.has_any_role_in_org_exact(_user_id uuid, _roles app_role[], _org uuid)
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
...
REVOKE ALL ON FUNCTION public.has_any_role_in_org_exact(uuid, app_role[], uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.has_any_role_in_org_exact(uuid, app_role[], uuid) TO authenticated, service_role;

COMMENT ON FUNCTION public.has_any_role_in_org_exact(uuid, app_role[], uuid) IS
  'FIX B-6 (Ola 8): como has_any_role_in_org pero SIN expansión de roles_jerarquia — la membresía debe tener exactamente uno de _roles. Reservado para paridad con listas cerradas legacy (pilotos financieros). super_admin conserva bypass de plataforma.';
