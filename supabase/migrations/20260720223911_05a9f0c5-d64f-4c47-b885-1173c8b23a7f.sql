-- Backfill: convert UUID values in embarque_contenedores.tipo_contenedor to human-readable code.
UPDATE public.embarque_contenedores ec
SET tipo_contenedor = tc.code
FROM public.tipos_contenedor tc
WHERE ec.tipo_contenedor = tc.id::text;

-- Fix RPC to store code (not UUID) in embarque_contenedores.tipo_contenedor going forward.
...
BEGIN
  ...
  -- Resolve tipo_contenedor: cotizaciones stores UUID from catálogo; embarque_contenedores stores CODE.
  v_tipo_cont_code := NULL;
  IF v_cot.tipo_contenedor IS NOT NULL AND v_cot.tipo_contenedor <> '' THEN
    IF v_cot.tipo_contenedor ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$' THEN
      SELECT code INTO v_tipo_cont_code FROM public.tipos_contenedor WHERE id = v_cot.tipo_contenedor::uuid;
    END IF;
    IF v_tipo_cont_code IS NULL THEN
  ...
  VALUES (
    ...
    v_cot.operador, v_cot.tipo_carga, COALESCE(v_tipo_cont_code, v_cot.tipo_contenedor),
