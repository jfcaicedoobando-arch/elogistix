-- v13.146.0
-- 1) convertir_proformas_a_factura: ya NO reserva folio interno.
--    El folio se asigna al timbrar (FacturAPI = source of truth).
...
BEGIN
  ...
  -- v13.146.0: NO se reserva folio interno; el folio real lo asigna FacturAPI al timbrar.
  -- Se guarda un `numero` placeholder único para respetar NOT NULL. El folio_fiscal
  -- queda en NULL y `serie` se guarda como preferencia (editable) pero se sobreescribe
  -- con la serie que FacturAPI devuelva.
...
-- 2) Nuevo RPC: eliminar borrador y revertir proformas.
...
BEGIN
  ...
  -- Recolecta proformas ligadas (por FK single o por la columna en la proforma).
  SELECT COALESCE(array_agg(id), ARRAY[]::uuid[])
    INTO v_proforma_ids
  FROM public.proformas
  WHERE factura_id = p_factura_id;

  -- Revertir proformas al estado previo a la conversión.
  UPDATE public.proformas
     SET factura_id        = NULL,
...
REVOKE ALL ON FUNCTION public.eliminar_factura_borrador(uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.eliminar_factura_borrador(uuid) TO authenticated;