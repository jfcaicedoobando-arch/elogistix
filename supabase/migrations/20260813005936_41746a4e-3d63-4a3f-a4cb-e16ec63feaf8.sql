...
BEGIN
  ...
  clasificado AS (
    SELECT s.moneda, s.saldo,
           CASE
             WHEN s.fecha_vencimiento IS NULL OR s.fecha_vencimiento >= CURRENT_DATE THEN 'Vigente'
             WHEN CURRENT_DATE - s.fecha_vencimiento <= 30 THEN '1-30'
             WHEN CURRENT_DATE - s.fecha_vencimiento <= 60 THEN '31-60'
             WHEN CURRENT_DATE - s.fecha_vencimiento <= 90 THEN '61-90'
             ELSE '90+'
           END AS bucket
...
REVOKE ALL ON FUNCTION public.proveedor_estado_cuenta_movimientos(uuid, date, date) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.proveedor_estado_cuenta_movimientos(uuid, date, date) FROM anon;
GRANT EXECUTE ON FUNCTION public.proveedor_estado_cuenta_movimientos(uuid, date, date) TO authenticated;
GRANT EXECUTE ON FUNCTION public.proveedor_estado_cuenta_movimientos(uuid, date, date) TO service_role;