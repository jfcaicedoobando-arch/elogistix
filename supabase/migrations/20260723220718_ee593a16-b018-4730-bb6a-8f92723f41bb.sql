-- FIX-R3-01: corrige path UPDATE del guard de pagos a proveedor.
-- Bug: comparaba delta (NEW - OLD) contra un saldo que ya excluía la propia
-- fila (id <> NEW.id), permitiendo sobrepago de hasta OLD.monto en UPDATE.
-- Fix: valida NEW.monto_en_moneda_factura directo contra v_saldo (INSERT y UPDATE).
