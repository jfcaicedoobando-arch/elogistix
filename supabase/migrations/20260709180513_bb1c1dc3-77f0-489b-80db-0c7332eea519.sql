...
ALTER TABLE public.conceptos_factura
  ADD COLUMN IF NOT EXISTS embarque_id uuid REFERENCES public.embarques(id) ON DELETE SET NULL,
  ADD COLUMN IF NOT EXISTS proforma_id_origen uuid REFERENCES public.proformas(id) ON DELETE SET NULL;

CREATE INDEX IF NOT EXISTS idx_conceptos_factura_embarque_id
  ON public.conceptos_factura (factura_id, embarque_id);

-- B. Tabla puente factura ↔ embarques
CREATE TABLE IF NOT EXISTS public.factura_embarques (
  factura_id uuid NOT NULL REFERENCES public.facturas(id) ON DELETE CASCADE,
  embarque_id uuid NOT NULL REFERENCES public.embarques(id) ON DELETE CASCADE,
  organization_id uuid NOT NULL REFERENCES public.organizations(id),
  created_at timestamptz NOT NULL DEFAULT now(),
  PRIMARY KEY (factura_id, embarque_id)
);

GRANT SELECT, INSERT, UPDATE, DELETE ON public.factura_embarques TO authenticated;
GRANT ALL ON public.factura_embarques TO service_role;

ALTER TABLE public.factura_embarques ENABLE ROW LEVEL SECURITY;

DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='factura_embarques' AND policyname='Tenant read factura_embarques') THEN
    CREATE POLICY "Tenant read factura_embarques"
      ON public.factura_embarques FOR SELECT TO authenticated
      USING (organization_id = public.current_user_org_id()
             OR public.has_role(auth.uid(), 'super_admin'::app_role));
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='factura_embarques' AND policyname='Tenant write factura_embarques') THEN
    CREATE POLICY "Tenant write factura_embarques"
      ON public.factura_embarques FOR ALL TO authenticated
      USING (organization_id = public.current_user_org_id()
             OR public.has_role(auth.uid(), 'super_admin'::app_role))
...
CREATE INDEX IF NOT EXISTS idx_factura_embarques_embarque
  ON public.factura_embarques (embarque_id);

-- C. Cobro cliente
ALTER TABLE public.embarques
  ADD COLUMN IF NOT EXISTS cobro_cliente_status text NOT NULL DEFAULT 'pendiente',
  ADD COLUMN IF NOT EXISTS cobro_cliente_actualizado_at timestamptz;

DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname='embarques_cobro_cliente_status_check') THEN
    ALTER TABLE public.embarques
      ADD CONSTRAINT embarques_cobro_cliente_status_check
      CHECK (cobro_cliente_status IN ('pendiente','parcial','pagado'));
  END IF;
END $$;

-- D. Recalcular con bypass del bloqueo de embarque cerrado
...
BEGIN
  ...
  -- El estado de cobro es una dimensión operativamente independiente:
  -- un embarque cerrado sigue pudiendo cobrarse.
  PERFORM set_config('app.bypass_cierre', 'on', true);

  FOREACH v_emb IN ARRAY p_embarque_ids LOOP
    SELECT
      count(*) FILTER (WHERE f.estado <> 'Cancelada'::estado_factura),
...
CREATE OR REPLACE FUNCTION public.trg_facturas_estado_a_embarques()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  ...
  IF v_ids IS NOT NULL THEN
    PERFORM public.recalcular_cobro_embarques(v_ids);
  END IF;
...
DROP TRIGGER IF EXISTS trg_facturas_estado_a_embarques ON public.facturas;
CREATE TRIGGER trg_facturas_estado_a_embarques
  AFTER UPDATE OF estado ON public.facturas
  FOR EACH ROW
  WHEN (OLD.estado IS DISTINCT FROM NEW.estado)
  EXECUTE FUNCTION public.trg_facturas_estado_a_embarques();

-- E. RPC convertir_proformas_a_factura (poblar trazabilidad + puente)
CREATE OR REPLACE FUNCTION public.convertir_proformas_a_factura(
  p_proforma_ids uuid[], p_serie_id uuid, p_metodo_pago text, p_forma_pago text,
  p_uso_cfdi text, p_dias_credito integer DEFAULT NULL::integer,
  p_notas text DEFAULT NULL::text, p_request_id uuid DEFAULT NULL::uuid
)
...
-- F. Backfill
INSERT INTO public.factura_embarques (factura_id, embarque_id, organization_id)
SELECT f.id, f.embarque_id, f.organization_id
FROM public.facturas f
WHERE f.embarque_id IS NOT NULL
...
DECLARE
  v_ids uuid[];
BEGIN
  SELECT array_agg(DISTINCT embarque_id) INTO v_ids FROM public.factura_embarques;
  IF v_ids IS NOT NULL THEN
    PERFORM public.recalcular_cobro_embarques(v_ids);
  END IF;
END $$;
