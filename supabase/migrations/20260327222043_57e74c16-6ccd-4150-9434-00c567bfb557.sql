...
BEGIN
  WITH embarques_base AS (
    SELECT e.id, e.expediente, e.cliente_nombre, e.modo::text, e.tipo::text,
  ...
  -- 2. Alertas demora (Arribo + diasDemora >= 0)
  alertas AS (
    SELECT jsonb_agg(
      jsonb_build_object(
  ...
  -- 3. Próximos arribos (En Tránsito, eta en próximos 7 días)
  proximos AS (
    SELECT jsonb_agg(
      jsonb_build_object(
        'id', a.id, 'expediente', a.expediente, 'cliente_nombre', a.cliente_nombre,
  ...
  FROM conteo, alertas, proximos, profit_este_mes, arribos_mes, mes_sig, resumen_sig, total_activos ta;
