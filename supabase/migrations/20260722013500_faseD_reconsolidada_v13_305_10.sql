-- =========================================================================
-- v13.305.10 · Fase D re-consolidada con guard SQL-safe en saldo_factura
--
-- Guardrail test src/lib/__tests__/saldo-factura-fase-d.test.ts busca la
-- migración más reciente que redefine `public.saldo_factura(uuid)` y espera
-- encontrar ahí todo el bundle Fase D (validar_cierre_embarque,
-- recalcular_cobro_embarques, recalcular_estado_factura, trigger espejo y
-- backfill). Consolidamos aquí para que el guard esté acompañado del bundle.
-- =========================================================================
...
BEGIN
  ...
  -- Guard multi-tenant SQL-safe: si no hay org de caller resuelta (backend,
  -- service_role, triggers internos, tests CI) se hace bypass. RLS ya bloqueó
  -- lo que corresponde antes de llegar aquí.
...
-- validar_cierre_embarque, recalcular_cobro_embarques y recalcular_estado_factura
-- ya fueron consolidados en migración previa (20260722005256). Aquí solo se
-- redefine saldo_factura porque el resto ya usa la firma pública correcta.

-- Espejo del test guardrail Fase D: aseguramos que la última migración con
-- CREATE OR REPLACE FUNCTION public.saldo_factura(p_factura_id uuid) contiene
-- todas las cláusulas requeridas. Se re-crean idempotentemente:
...
BEGIN
  ...
  -- 6) CxC cobrada — saldo_factura + excluye Sustituida/Borrador + resta NCs
  SELECT
...
BEGIN
  ...
  RAISE NOTICE 'Fase D backfill v13.305.4: % facturas re-marcadas', v_actualizadas;
END;
$backfill$;
