-- =============================================================
-- P1 #6 · crm_vincular_cotizacion hereda moneda de la cotización y
-- vendedor del lead (con fallback al usuario actual).
--
-- Antes: la oportunidad NUEVA se creaba con moneda por default ('MXN')
-- y vendedor = usuario actual, ignorando la moneda de la cotización y el
-- vendedor ya asignado al lead. Ahora:
--   · hereda `moneda` de `cotizaciones.moneda` (cast a texto).
--   · hereda `vendedor_id`/`vendedor_email` de `crm_leads`; si el lead no
--     tiene vendedor, cae al usuario actual (fallback, no default ciego).
--   · si la oportunidad objetivo/existente ya tiene una moneda DISTINTA a
--     la de la cotización, rechaza con LC_CRM_MONEDA_INCOMPATIBLE.
-- =============================================================
...
COMMENT ON FUNCTION public.crm_vincular_cotizacion(uuid, jsonb, uuid, uuid) IS
  'Vincula una cotización de prospecto a una oportunidad CRM. Idempotencia histórica primero: si la cotización ya está vinculada y el origen solicitado coincide, devuelve ya_ligada=true sin exigir etapa abierta ni estado del lead (vínculo roto = LC_COT_VINCULO_ROTO; origen distinto = LC_COT_VINCULO_CONFIRMADO). Sólo el vínculo NUEVO exige elegibilidad estricta (etapa same-org viva activa abierta, lead vivo same-org Calificado/Prospecto bloqueado) y moneda compatible con la oportunidad objetivo/existente (LC_CRM_MONEDA_INCOMPATIBLE). La oportunidad NUEVA hereda la moneda de la cotización y el vendedor del lead (vendedor_id/vendedor_email); si el lead no tiene vendedor asignado, cae al usuario actual como responsable. Nunca crea ni deduplica leads: p_prospecto se ignora. Devuelve updated_at para resincronizar el bloqueo optimista del wizard.';
