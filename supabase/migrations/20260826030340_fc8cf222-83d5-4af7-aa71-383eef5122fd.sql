-- B-06: `reabrir_embarque` escribía `p_usuario_email` (texto enviado por el
-- cliente) en notas_embarque y eventos_embarque, permitiendo firmar la bitácora
-- con cualquier identidad. El actor ahora se deriva SIEMPRE de la sesión
-- autenticada (auth.uid() -> auth.users.email). El parámetro se conserva en la
-- firma por compatibilidad (no puede recibir DEFAULT por su posición) pero se
-- ignora.
