-- v13.147.1: soportar proformas 100% USD al convertir a factura borrador.
-- Antes: siempre se insertaba la factura en MXN con montos MXN. Si la proforma
-- era 100% USD (subtotal_mxn = 0), el tipo_cambio calculado quedaba en 0 y
-- violaba el check `facturas_tipo_cambio_pos`.
...
BEGIN
  ...
  -- v13.147.1: seleccionar moneda del borrador según los importes reales.
  -- Si es 100% USD, la factura queda en USD (tipo_cambio = 1 como placeholder;
  -- FacturAPI asigna el tipo de cambio DOF real al timbrar).
  IF v_subtotal_mxn > 0 AND v_subtotal_usd = 0 THEN
    v_moneda := 'MXN';
    v_subtotal := v_subtotal_mxn; v_iva := v_iva_mxn; v_total := v_total_mxn;
    v_tipo_cambio := 1;
  ELSIF v_subtotal_usd > 0 AND v_subtotal_mxn = 0 THEN
    v_moneda := 'USD';
    v_subtotal := v_subtotal_usd; v_iva := v_iva_usd; v_total := v_total_usd;
    v_tipo_cambio := 1;
  ELSIF v_subtotal_mxn > 0 AND v_subtotal_usd > 0 THEN
    v_moneda := 'MXN';
    v_subtotal := v_subtotal_mxn; v_iva := v_iva_mxn; v_total := v_total_mxn;
    v_tipo_cambio := ROUND(v_subtotal_mxn / v_subtotal_usd, 4);
    IF v_tipo_cambio <= 0 THEN v_tipo_cambio := 1; END IF;
  ELSE
    RAISE EXCEPTION 'La proforma no tiene importes para facturar';
  END IF;
  ...
  INSERT INTO public.facturas (
    ...
    v_subtotal, v_iva, v_total, v_moneda, v_tipo_cambio,
    CURRENT_DATE,
