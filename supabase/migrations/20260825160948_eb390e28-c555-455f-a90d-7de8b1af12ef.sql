ALTER TABLE public.embarque_facturas_entrantes
  ADD COLUMN IF NOT EXISTS subtotal_detectado numeric;

COMMENT ON COLUMN public.embarque_facturas_entrantes.subtotal_detectado IS
  'Subtotal (sin impuestos) del CFDI. Es el importe comparable contra los costos del ERP.';
...
CREATE OR REPLACE FUNCTION public.adjuntar_xml_entrante_verificado(
  ...
  p_xml_nombre text,
  p_xml_hash text,
  p_uuid_fiscal text DEFAULT NULL::text,
  p_rfc_emisor text DEFAULT NULL::text,
  p_folio_serie text DEFAULT NULL::text,
  p_fecha_emision date DEFAULT NULL::date,
  p_total_detectado numeric DEFAULT NULL::numeric,
  p_moneda_detectada text DEFAULT NULL::text,
  p_subtotal_detectado numeric DEFAULT NULL::numeric
)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
...
REVOKE ALL ON FUNCTION public.adjuntar_xml_entrante_verificado(uuid, uuid, text, text, text, text, text, text, date, numeric, text, numeric) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.adjuntar_xml_entrante_verificado(uuid, uuid, text, text, text, text, text, text, date, numeric, text, numeric) TO authenticated, service_role;
