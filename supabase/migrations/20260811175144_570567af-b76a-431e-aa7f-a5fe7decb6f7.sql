...
BEGIN
  ...
  PERFORM public.idempotency_store(p_request_id, 'actualizar_embarque_completo', v_resp);
  RETURN v_resp;
END;
$function$;

-- v13.509.0 · Replicación de costos desde cotización: además del nombre,
-- resolvemos el proveedor del catálogo (por nombre normalizado o alias) para
-- que el vínculo quede firme desde la creación del embarque.
...
AS $function$
  SELECT p.id
   ...
   WHERE p.organization_id = p_org
     ...
     AND upper(btrim(p.nombre)) = upper(btrim(COALESCE(p_nombre, '')))
     AND btrim(COALESCE(p_nombre, '')) <> ''
...
-- Backfill idempotente: repone el proveedor perdido desde la cotización de
-- origen (match por concepto dentro del mismo embarque) y resuelve el id.
UPDATE public.conceptos_costo cc
   SET proveedor_nombre = src.proveedor
  FROM public.embarques e
  JOIN public.cotizacion_costos src
    ON src.cotizacion_id = e.cotizacion_id
 ...
 WHERE cc.embarque_id = e.id
   ...
   AND upper(btrim(src.concepto)) = upper(btrim(cc.concepto))
   AND COALESCE(btrim(src.proveedor), '') <> '';
