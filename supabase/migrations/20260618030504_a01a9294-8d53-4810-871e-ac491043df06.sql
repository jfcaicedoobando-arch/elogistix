...
-- =============================================================
-- v13.66.11 — Demoras por contenedor
-- =============================================================

-- 1) Columnas nuevas en embarque_contenedores
ALTER TABLE public.embarque_contenedores
  ADD COLUMN IF NOT EXISTS fecha_descarga date NULL,
  ADD COLUMN IF NOT EXISTS fecha_devolucion date NULL,
  ADD COLUMN IF NOT EXISTS dias_libres_override integer NULL;

-- 2) Reescritura de calcular_demoras_embarque con fechas por contenedor + fallback
...
BEGIN
  ...
  -- Condiciones naviera (default de días libres)
  ...
  -- Iterar contenedores con cálculo individual
  FOR v_contenedor IN
  ...
  LOOP
    ...
    -- Fechas por contenedor con fallback al timeline
...
-- 3) Trigger: recalcular cuando cambien fechas/override por contenedor
CREATE OR REPLACE FUNCTION public.trg_recalcular_demoras_contenedor()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  IF (
    NEW.fecha_descarga IS DISTINCT FROM OLD.fecha_descarga
    OR NEW.fecha_devolucion IS DISTINCT FROM OLD.fecha_devolucion
    OR NEW.dias_libres_override IS DISTINCT FROM OLD.dias_libres_override
  ) THEN
    PERFORM public.calcular_demoras_embarque(NEW.embarque_id);
  END IF;
  RETURN NEW;
END;
$$;

REVOKE ALL ON FUNCTION public.trg_recalcular_demoras_contenedor() FROM PUBLIC, anon;

DROP TRIGGER IF EXISTS trg_contenedor_demoras_recalc ON public.embarque_contenedores;
CREATE TRIGGER trg_contenedor_demoras_recalc
  AFTER UPDATE OF fecha_descarga, fecha_devolucion, dias_libres_override
  ON public.embarque_contenedores
  FOR EACH ROW EXECUTE FUNCTION public.trg_recalcular_demoras_contenedor();
