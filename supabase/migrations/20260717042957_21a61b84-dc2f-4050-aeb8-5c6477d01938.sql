-- v13.301.56 — Hardening de provision_organization
-- 1. REVOKE explícito de anon y public (defensa en profundidad)
-- 2. Log de auditoría en bitacora_actividad en cada aprovisionamiento
-- 3. Validaciones adicionales (nombre longitud, evitar duplicado exacto por nombre+rfc)
-- 4. Verificación temprana con SECURITY INVOKER-like: comprueba super_admin ANTES
--    de tocar cualquier tabla para minimizar side-effects si falla.
...
BEGIN
  -- Autorización estricta: caller autenticado + super_admin.
  ...
  -- Validaciones de payload (después de la autorización, no antes).
  IF v_nombre = '' THEN
  ...
  -- Evitar duplicados exactos (nombre + rfc) en la misma pasada.
  IF EXISTS (
  ...
  -- Auditoría: bypass RLS porque estamos en SECURITY DEFINER.
...
REVOKE ALL ON FUNCTION public.provision_organization(text, text, uuid) FROM anon;
...
COMMENT ON FUNCTION public.provision_organization(text, text, uuid) IS
  'v13.301.56 — Aprovisiona una organización nueva. Sólo super_admin. Registra en bitacora_actividad.';
