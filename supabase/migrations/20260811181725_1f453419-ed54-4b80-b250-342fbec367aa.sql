...
BEGIN
  ...
  -- v13.509.4 · antes se filtraba por una columna inexistente
  -- (conceptos_costo.proveedor_factura_id), lo que abortaba todo el guardado
  -- con 42703. El vínculo real vive en proveedor_facturas_conceptos.
...
REVOKE ALL ON FUNCTION public.actualizar_embarque_completo(uuid, jsonb, jsonb, jsonb, uuid, timestamptz) FROM anon;
GRANT EXECUTE ON FUNCTION public.actualizar_embarque_completo(uuid, jsonb, jsonb, jsonb, uuid, timestamptz) TO authenticated;
GRANT EXECUTE ON FUNCTION public.actualizar_embarque_completo(uuid, jsonb, jsonb, jsonb, uuid, timestamptz) TO service_role;