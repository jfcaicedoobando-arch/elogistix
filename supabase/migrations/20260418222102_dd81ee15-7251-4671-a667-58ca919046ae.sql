-- Split dashboard_stats into two RPCs for performance:
--   1) dashboard_summary  → KPIs + conteo + arribos del mes + resumen mes siguiente (cargado eager)
--   2) dashboard_details  → listas largas (alertas, próximos, profit, mes siguiente, cargas) (cargado lazy)
...
BEGIN
  RETURN (
    ...
    profit AS (SELECT p.embarque_id, p.venta_usd, p.costo_usd FROM profit_por_embarque() p),
...
BEGIN
  RETURN (
    ...
    profit_este_mes_src AS (
      ...
      ORDER BY (COALESCE(p.venta_usd, 0) - COALESCE(p.costo_usd, 0)) DESC LIMIT 30
    ),
    profit_este_mes AS (
      SELECT jsonb_agg(jsonb_build_object(
    ...
    cargas_cliente AS (
      ...
      FROM (
        SELECT jsonb_build_object(
          'cliente_id', cliente_id, 'cliente_nombre', cliente_nombre, 'total', count(*)
        ) AS x
