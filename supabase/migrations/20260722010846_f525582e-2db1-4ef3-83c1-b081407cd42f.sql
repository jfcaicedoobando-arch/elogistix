-- Fase D v13.305.4 — saldo_factura permite contexto backend (auth.uid() NULL)
-- Corrige regresión: triggers de integridad (tg_pago_factura_no_sobrepago) y
-- tests CI que ejecutan como postgres/service_role veían saldo=0 y bloqueaban
-- inserts legítimos con LC_PAGO_EXCEDE_SALDO.
...
BEGIN
  ...
  -- v13.305.4: bypass cuando auth.uid() IS NULL — indica contexto backend
  -- confiable (postgres, service_role, triggers internos, tests CI) donde
  -- RLS ya se encargó de filtrar. Sin este bypass, triggers como
  -- tg_pago_factura_no_sobrepago rompen inserts legítimos.
