...
-- RNF-01 (idempotencia) y RNF-02 (cuadre exacto).
-- ============================================================
...
BEGIN
  ...
  -- Ola 11 · RNF-01: reclamo atómico de la llave de idempotencia.
  ...
  IF v_importe IS NULL OR v_importe <= 0 THEN
    RAISE EXCEPTION 'LC_COBRO_LOTE_IMPORTE_REQUERIDO: Captura el importe recibido del cliente.'
      USING ERRCODE = '42501';
  END IF;
  -- Ola 11 · RNF-02: cuadre exacto tras ROUND a 2 decimales.
...
-- ============================================================
-- Ola 11 · RNF-05/RNF-06 (espejo de CxC) en el pago en lote a proveedor.
-- ACUMULATIVA: fecha (RFE-02/RNF-03) + importe/cuadre exacto + duplicados.
-- ============================================================
...
BEGIN
  ...
  -- Ola 11 · RNF-06 (espejo RG4-6): una misma factura no puede repetirse.
  IF EXISTS (
    SELECT 1
    FROM jsonb_array_elements(COALESCE(p_payload->'renglones','[]'::jsonb)) AS r
    GROUP BY (r->>'factura_id')
