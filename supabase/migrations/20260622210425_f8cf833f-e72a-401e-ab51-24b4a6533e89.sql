-- 1) Enum tipo contable
DO $$ BEGIN
  CREATE TYPE public.tipo_contable_categoria AS ENUM ('CostoDirectoEmbarque','IndirectoOperacion','Administracion');
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- 2) Columna en presupuesto_categorias
ALTER TABLE public.presupuesto_categorias
  ADD COLUMN IF NOT EXISTS tipo_contable public.tipo_contable_categoria NOT NULL DEFAULT 'Administracion';

-- 3) Backfill por nombre
UPDATE public.presupuesto_categorias
SET tipo_contable = 'IndirectoOperacion'
WHERE lower(nombre) IN ('nómina','nomina');

UPDATE public.presupuesto_categorias
SET tipo_contable = 'CostoDirectoEmbarque'
WHERE lower(nombre) = 'comisiones';

-- 4) Actualizar dashboard_summary para usar la categoría de la factura
