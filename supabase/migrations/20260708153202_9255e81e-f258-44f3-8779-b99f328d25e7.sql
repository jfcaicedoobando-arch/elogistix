...
BEGIN
  ...
  -- Fuente 3: bitácora `factura.borrador_generado` — caso consolidado.
  -- v13.217.1 — `jsonb_array_elements_text` devuelve `text`; hay que castear
  -- a `uuid` antes de `array_agg` para no romper el COALESCE con `uuid[]`.
  SELECT COALESCE(
    array_agg(DISTINCT pid::uuid),
    ARRAY[]::uuid[]
  )
    INTO v_bitacora_ids
