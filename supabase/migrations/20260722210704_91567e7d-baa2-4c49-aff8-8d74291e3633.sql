...
BEGIN
  ...
  -- ¿Tiene líneas "puras" (XML / captura directa: concepto_costo_id NULL)?
  SELECT EXISTS(
  ...
  IF v_tiene_xml_lineas THEN
    -- Modo XML/manual: sólo las líneas de la factura cuentan.
    -- Los renglones con concepto_costo_id son espejos de costos de embarque y no deben duplicar el subtotal.
  ...
  ELSE
    -- Modo captura por vínculo a embarque (sin XML): suma todos los renglones (incluye ajustes).
    SELECT COUNT(*), COALESCE(SUM(monto * COALESCE(NULLIF(cantidad,0),1)),0)
      INTO v_conceptos_count, v_suma_conceptos
      FROM public.proveedor_facturas_conceptos
      WHERE proveedor_factura_id = p_factura_id;
