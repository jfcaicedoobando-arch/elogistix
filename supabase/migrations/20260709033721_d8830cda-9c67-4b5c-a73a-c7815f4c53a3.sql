...
BEGIN
  ...
  exigidos AS (
    ...
    CROSS JOIN LATERAL (
      VALUES
        ...
        ('Arribo',
          CASE e.modo::text
            WHEN 'Aéreo'     THEN ARRAY['Factura Comercial','Packing List','Air Waybill (AWB)','Certificado de Origen','Ficha Técnica','Pedimento']
            WHEN 'Terrestre' THEN ARRAY['Factura','Lista de Empaque','Carta Porte','Pedimento']
            ELSE                  ARRAY['Factura Comercial','Packing List','Bill of Lading (BL Master)','Bill of Lading (BL House)','Certificado de Origen','Ficha Técnica','Pedimento']
          END),
        ('Entregado',
          CASE e.modo::text
            WHEN 'Aéreo'     THEN ARRAY['Factura Comercial','Packing List','Air Waybill (AWB)','Certificado de Origen','Ficha Técnica','Pedimento','POD']
            WHEN 'Terrestre' THEN ARRAY['Factura','Lista de Empaque','Carta Porte','Pedimento','POD']
            ELSE                  ARRAY['Factura Comercial','Packing List','Bill of Lading (BL Master)','Bill of Lading (BL House)','Certificado de Origen','Ficha Técnica','Pedimento','POD']
          END)
    ) AS req(estado_req, docs)
    CROSS JOIN LATERAL unnest(req.docs) AS d(doc_nombre)
    WHERE e.estado::text = req.estado_req
  ),
  ...
  -- AUD-1 (v13.235.0): Reglas basadas en `embarque_contenedores` (fuente de verdad).
  -- Reemplazan la lectura de columnas legacy en `embarques` (peso_kg, volumen_m3, piezas)
  -- que hoy sólo se mantienen por trigger de compatibilidad.
