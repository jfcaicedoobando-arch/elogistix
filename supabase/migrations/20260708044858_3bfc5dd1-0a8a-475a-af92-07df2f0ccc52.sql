-- v13.214.6 — Candado servidor: no se puede avanzar a "Arribo" sin fecha_llegada_real.
-- La UI del tab Tracking sigue siendo el único flujo que registra la fecha; el botón
-- "Avanzar a Arribo" del header ya no puede saltarse este requisito.
...
BEGIN
  ...
  SELECT organization_id, fecha_llegada_real INTO v_org_id, v_flr
  ...
  -- v13.214.6 — Candado: Arribo requiere fecha_llegada_real registrada.
