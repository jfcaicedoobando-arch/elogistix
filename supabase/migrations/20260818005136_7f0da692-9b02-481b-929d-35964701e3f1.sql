-- BUG-03: folio de proforma con advisory lock por organización
...
BEGIN
  IF p_org_id IS NULL
     OR (p_org_id IS DISTINCT FROM current_user_org_id()
         AND NOT has_role(auth.uid(), 'super_admin'::app_role)) THEN
    RAISE EXCEPTION 'Forbidden: organization mismatch';
  END IF;

  -- Serializa la generación de folios por (org, año) durante la transacción.
  PERFORM pg_advisory_xact_lock(hashtext('proforma_folio:' || p_org_id::text || ':' || v_year::text));
...
REVOKE ALL ON FUNCTION public.generar_numero_proforma(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.generar_numero_proforma(uuid) TO authenticated, service_role;

-- BUG-03: no permitir "robar" conceptos ya asignados a otra proforma
...
-- BUG-05: máquina de estados y UUID fiscal obligatorio en NC de cliente
...
BEGIN
  ...
  IF TG_OP = 'UPDATE' THEN
    ...
    IF v_old IS DISTINCT FROM v_new THEN
      IF NOT (
        (v_old = 'Borrador'  AND v_new IN ('Aprobada','Cancelada')) OR
        (v_old = 'Aprobada'  AND v_new IN ('Timbrada','Cancelada')) OR
        (v_old = 'Timbrada'  AND v_new IN ('Aplicada','Cancelada')) OR
        (v_old = 'Aplicada'  AND v_new = 'Cancelada')
      ) THEN
...
DROP TRIGGER IF EXISTS trg_nc_cliente_transicion ON public.factura_notas_credito;
CREATE TRIGGER trg_nc_cliente_transicion
BEFORE INSERT OR UPDATE ON public.factura_notas_credito
FOR EACH ROW EXECUTE FUNCTION public.guard_nc_cliente_transicion();