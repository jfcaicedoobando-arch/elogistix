...
-- ============================================================================
-- QA ronda 2 · D-02 (alto) + R-02 (medio)
-- D-02: el guard de cancelacion con CxC/CxP solo vivia en la RPC
--       avanzar_estado_embarque; un UPDATE embarques SET estado='Cancelado'
--       directo lo eludia. Fix: trigger BEFORE UPDATE OF estado que aplica la
--       MISMA validacion (facturas vivas con saldo en 'Emitida','Vencida',
--       'Parcialmente pagada'; proveedor_facturas no canceladas) salvo que la
--       GUC transaccional app.via_rpc_estado='1' (la establece la RPC).
-- R-02: al cancelar via RPC se liberan las cotizaciones ligadas
--       (embarque_id=NULL, 'En operación'→'Aceptada') bajo la GUC
--       app.liberando_papelera, el bypass ya existente de
--       guard_estado_cotizacion para housekeeping.
-- ============================================================================
