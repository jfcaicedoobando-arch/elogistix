-- Alinear máquina de estados de cotizaciones con la UI.
-- La UI permite Aceptar/Rechazar directamente desde Borrador; el guard sólo
-- permitía Enviada→Aceptada. Se agrega Borrador→Aceptada como transición válida.
-- Además, el trigger de snapshot se dispara ahora también cuando se acepta
-- directo desde Borrador, para no perder trazabilidad de versión congelada.
...
-- Snapshot también cuando se acepta directo desde Borrador (sin pasar por Enviada)
...
BEGIN
  -- Congelar snapshot al transitar a 'Enviada' o cuando se acepta directo desde 'Borrador'
  IF (OLD.estado IS DISTINCT FROM NEW.estado) THEN
    IF NEW.estado::text = 'Enviada' THEN
      v_should_snapshot := true;
    ELSIF NEW.estado::text = 'Aceptada' AND OLD.estado::text = 'Borrador' THEN
      v_should_snapshot := true;
    END IF;
  END IF;
