-- Ola 4 · N12: aplicar_anticipo_a_factura y cancelar_anticipo_proveedor
-- no validaban la org del llamador (user_roles es GLOBAL) → cualquier rol
-- financiero podía mover saldos de OTRA organización. Se añade el mismo
-- guard que ya usa vincular_anticipo_embarque (LC_ANTICIPO_OTRA_ORG),
-- con IS DISTINCT FROM para fallar cerrado cuando current_user_org_id()
-- es NULL.
...
BEGIN
  ...
  -- Ola 4 · N12: guard de tenant. Antes bastaba un rol financiero GLOBAL
  -- para aplicar anticipos de otra organización (user_roles es única por
  -- usuario). Mismo código que vincular_anticipo_embarque
  -- (LC_ANTICIPO_OTRA_ORG, 20260808002508), con IS DISTINCT FROM para
  -- fallar cerrado cuando current_user_org_id() es NULL.
...
BEGIN
  ...
  -- Ola 4 · N12: guard de tenant (ver comentario en aplicar_anticipo_a_factura).
