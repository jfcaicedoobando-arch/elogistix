...
-- FIX-R4-10a residual: fail-closed en validar_cierre_embarque.
-- Antes: `IF v_caller_org IS NOT NULL AND v_emb.org <> v_caller_org AND NOT super_admin`
-- permitía a usuarios autenticados sin membresía (caller_org NULL) leer el
-- JSON financiero de cualquier embarque. Ahora exige membresía real salvo
-- super_admin o service_role.
...
BEGIN
  ...
  -- FIX-R4-10a: fail-closed. Sólo service_role o super_admin bypasean org check.
  ...
  WITH agg AS (
    SELECT COALESCE(pf.moneda,'MXN') AS moneda,
  ...
  WITH agg AS (
    SELECT COALESCE(f.moneda,'MXN') AS moneda,
      COALESCE(SUM(f.total),0) AS total,
      COALESCE(SUM(public.saldo_factura(f.id)),0) AS saldo,
      COALESCE(SUM((
        SELECT COALESCE(SUM(pf.monto_aplicado_factura),0)
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
      'moneda', moneda, 'total', total, 'pagado', pagado,
      'notas_credito', notas_credito, 'saldo', GREATEST(saldo, 0),
      'facturas_pendientes', facturas_pendientes
    ) ORDER BY moneda), '[]'::jsonb),
    COALESCE(SUM(GREATEST(saldo, 0)), 0)
  INTO v_cxc_por_moneda, v_cxc_saldo
  FROM agg;
