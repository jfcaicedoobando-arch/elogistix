-- ============================================================================
-- FIX-AGING-NC-01 (auditoría de tests 2026-07-24, hallazgo H1)
--
-- `cxp_aging_proveedores`: el CTE `nc` no filtraba `deleted_at IS NULL`,
-- así que una nota de crédito aplicada y luego soft-borrada seguía
-- descontando saldo en el reporte de aging — divergencia con el guard de
-- sobrepago (`tg_pago_proveedor_no_sobrepago`, que sí filtra) y con la
-- vista canónica `v_proveedor_facturas_saldo` (que sí filtra).
--
-- Cambio quirúrgico: `AND deleted_at IS NULL` en el CTE nc.
-- Test conductual que lo fija: supabase/tests/aging_nc_deleted_at.sql
-- (falla contra la versión pre-fix; pasa con esta migración).
-- ============================================================================
...
BEGIN
  ...
  WITH pagado AS (
    ...
    -- FIX-AGING-NC-01: ignorar NCs soft-borradas (paridad con el guard
    -- de sobrepago y con v_proveedor_facturas_saldo).
