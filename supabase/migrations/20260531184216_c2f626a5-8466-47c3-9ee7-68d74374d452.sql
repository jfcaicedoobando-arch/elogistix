...
-- 1) Añadir valor 'Borrador' al enum estado_embarque
ALTER TYPE public.estado_embarque ADD VALUE IF NOT EXISTS 'Borrador' BEFORE 'Confirmado';

-- 2) Trigger sync_cotizacion_embarque_link: ignorar embarques en 'Borrador'
...
-- 3) RPC: crear_embarque_borrador_desde_cotizacion
...
BEGIN
  ...
  -- 2) Validaciones de permisos y reglas
  ...
  -- 6) Crear contenedores hijos repartiendo peso/volumen/piezas
  ...
  -- 7) Conceptos de costo: BL una vez (contenedor_id NULL), Contenedor por hijo
  FOR v_costo IN
  ...
  -- 8) Conceptos de venta desde jsonb
  ...
  -- 9) Vincular cotización al embarque borrador (sin cambiar estado: trigger ya respeta 'Borrador')
  UPDATE public.cotizaciones
  ...
  -- 11) Notificaciones in-app a admins/operadores de la org
...
GRANT EXECUTE ON FUNCTION public.crear_embarque_borrador_desde_cotizacion(uuid) TO authenticated;