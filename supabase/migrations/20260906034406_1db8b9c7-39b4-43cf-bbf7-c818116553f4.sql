...
BEGIN
  ...
  FOR v_row IN SELECT * FROM jsonb_array_elements(p_clientes) LOOP
    ...
    INSERT INTO public.clientes (
      ...
      v_nombre,
      v_rfc,
      btrim(COALESCE(v_row->>'direccion', '')),
      btrim(COALESCE(v_row->>'ciudad', '')),
      btrim(COALESCE(v_row->>'estado', '')),
      btrim(COALESCE(v_row->>'cp', '')),
      btrim(COALESCE(v_row->>'contacto', '')),
      btrim(COALESCE(v_row->>'telefono', '')),
