-- Ola 4 · N24: profit_por_cliente agrupaba por (cliente_id, cliente_nombre):
-- si el nombre capturado difiere entre embarques del mismo cliente (acento,
-- mayúsculas, razón social editada), el cliente salía partido en 2+ filas y
-- el dedupe del controlador frontend descartaba montos de las filas extra
-- (tabla != tarjetas KPI). Se agrupa sólo por cliente_id; el nombre
-- representativo se toma con MAX(cliente_nombre) — no se hace join a
-- `clientes` porque las filas con cliente_id NULL deben conservar su propio
-- grupo y `clientes.nombre` puede no aplicar ahí.
...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path TO 'public' AS $function$
  ...
  SELECT
    b.cliente_id,
    -- Ola 4 · N24: nombre representativo (no el más reciente necesariamente,
    -- pero estable) — evita partir al cliente por variaciones de captura.
