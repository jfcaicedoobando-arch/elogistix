-- ============================================================
-- BL-09: comisiones que quedaron en 0 por excepción tragada en
-- `calcular_comision_pago` (TC faltante o costos capturados tarde) no tenían
-- vía de recálculo y la liquidación tomaba lo que hubiera. Al cerrar el
-- embarque (momento en que la captura financiera ya se considera completa y
-- `cerrar_embarque` marca `definitiva = true`) se recalculan las comisiones
-- devengadas del embarque que tienen nota de pendiente. `calcular_comision_pago`
-- es idempotente (ON CONFLICT (pago_factura_id) DO UPDATE) y no toca las ya
-- 'Liquidada'. Cada recálculo se envuelve en su propio bloque para no abortar
-- el cierre si una comisión individual falla.
-- ACUMULATIVA: redefine `cerrar_embarque` 1:1 desde
-- 20260807211258_f1f5d7b2-98bb-4a11-a888-ba0848a00c6c.sql añadiendo sólo el
-- bloque de recálculo (marcado BL-09). Sin cambio de firma ni de grants.
-- ============================================================
...
-- Grants vigentes (20260807212527): se reafirman tras recrear la SECURITY DEFINER.
