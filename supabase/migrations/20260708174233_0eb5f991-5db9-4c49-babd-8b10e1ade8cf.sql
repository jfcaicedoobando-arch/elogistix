DROP FUNCTION IF EXISTS public.cxp_por_capturar();
...
CREATE OR REPLACE FUNCTION public.cxp_por_capturar()
 RETURNS TABLE(
   ...
   expediente text,
   cliente_nombre text,
   presupuestado_mxn numeric,
   presupuestado_usd numeric,
   facturado_mxn numeric,
   facturado_usd numeric,
   facturas_capturadas integer,
