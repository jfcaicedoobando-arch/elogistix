-- perf: Ola 1 · P8 — Resumen anual EERR en 1 RPC (2026-07-25)
-- Reemplaza 12 fetch mensuales de agregador.ts (tendencia dashboard ejecutivo)
-- por 1 llamada. Devuelve totales ya convertidos a MXN usando el TC del
-- embarque (fuente "embarques") o el TC de la propia factura como fallback
-- (fuente "facturas"), igual que hoy hacen fetchEstadoResultadosMes /
-- fetchEstadoResultadosDevengado en TS.
--
-- Semántica idéntica a `estadoResultados.ts` y `estadoResultadosDevengado.ts`
-- para los TOTALES (ingresos.total, costos.total). El desglose por concepto
-- y por modo NO se produce aquí — sigue calculándose en TS para el mes actual
-- y el mes previo, que sí necesitan el pivot completo.
CREATE OR REPLACE FUNCTION public.eerr_resumen_anual(
  p_year int,
  p_fuente text DEFAULT 'embarques'  -- 'embarques' | 'facturas'
)
RETURNS TABLE(
  mes int,
  ingresos_mxn numeric,
  costos_mxn numeric
)
LANGUAGE plpgsql
...
BEGIN
  ...
  IF p_fuente = 'embarques' THEN
    ...
    ing AS (
      SELECT em.mes,
        SUM(
          CASE UPPER(COALESCE(cv.moneda, 'MXN'))
    ...
    cst AS (
      SELECT em.mes,
        SUM(
          CASE UPPER(COALESCE(cc.moneda, 'MXN'))
  ...
  ELSIF p_fuente = 'facturas' THEN
    ...
    -- Facturas emitidas en el mes (vigentes): usa TC de la factura como
    -- fallback si no está vinculada al embarque via expediente.
    fact AS (
      SELECT
        EXTRACT(month FROM f.fecha_emision)::int AS mes,
        SUM(
          CASE UPPER(COALESCE(f.moneda, 'MXN'))
    ...
    -- NCs aplicadas: TC=1 (el TS también las trata así, mode Marítimo).
    ncs AS (
      SELECT
    ...
    -- Costos: proveedor_facturas emitidas en el mes.
    pfact AS (
      SELECT
        EXTRACT(month FROM pf.fecha_emision)::int AS mes,
        SUM(
          CASE UPPER(COALESCE(pf.moneda, 'MXN'))
...
GRANT EXECUTE ON FUNCTION public.eerr_resumen_anual(int, text) TO service_role;