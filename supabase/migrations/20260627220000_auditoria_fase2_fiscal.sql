...
DECLARE
  ...
  -- Fase 1 (auditoría modernización): fecha de inicio del nuevo modelo de
  -- facturación (FacturAPI / proforma → factura). Embarques con ETD anterior
  -- no se marcan como `ventas_sin_facturar` para evitar falsos positivos.
...
BEGIN
  ...
  WHERE categoria = 'auditoria' AND clave = 'dias_borrador_abandonado'
    AND organization_id = p_organization_id
  LIMIT 1;
  -- Fase 1: piso de 1 día para evitar que una configuración accidental con 0
  -- inunde la auditoría con borradores recién creados.
  ...
  hall_docs_pendientes AS (
    -- Fase 1 (auditoría modernización): severidad bajada de 'critico' → 'alto'.
    -- El candado `validar_cierre_embarque` ya bloquea cerrar con docs en
    -- pendiente, por lo que dejar este hallazgo como crítico inflaba el score
    -- duplicando una regla que el sistema ya impide.
    SELECT jsonb_build_object(
  ...
  hall_ventas AS (
    -- Fase 1 (auditoría modernización): excluye embarques con ETD anterior al
    -- inicio del nuevo modelo de facturación. Antes inundaba la auditoría con
    -- embarques históricos que se reconcilian vía /admin/auditoria backfill.
    SELECT jsonb_build_object(
      'embarque_id', e.id, 'expediente', e.expediente,
      'cliente_nombre', e.cliente_nombre, 'modo', e.modo::text, 'estado', e.estado::text, 'eta', e.eta,
  ...
  -- ============================================================================
  -- Fase 2 (auditoría modernización) — Reglas fiscales nuevas (FacturAPI).
  -- ============================================================================
  hall_factura_sin_timbrar AS (
    -- Factura creada hace > 48 h sin timbrar (sin uuid_fiscal y estado no Cancelada/Sustituida).
    SELECT jsonb_build_object(
      'embarque_id', e.id, 'expediente', e.expediente,
      'cliente_nombre', e.cliente_nombre, 'modo', e.modo::text, 'estado', e.estado::text, 'eta', e.eta,
      'regla', 'factura_sin_timbrar', 'severidad', 'alto',
      'detalle', 'Factura ' || f.numero || ' creada hace ' ||
                 EXTRACT(DAY FROM (now() - f.created_at))::int || ' día(s) sin timbrar (estado ' || f.estado::text || ')',
      'documentos_faltantes', '[]'::jsonb,
      'monto_mxn', (f.total * CASE
        WHEN f.moneda::text = 'MXN' THEN 1
  ...
  hall_rep_pendiente AS (
    -- Pago registrado para factura PPD con REP pendiente > 72 h.
    SELECT jsonb_build_object(
      'embarque_id', e.id, 'expediente', e.expediente,
      'cliente_nombre', e.cliente_nombre, 'modo', e.modo::text, 'estado', e.estado::text, 'eta', e.eta,
      'regla', 'rep_pendiente', 'severidad', 'alto',
      'detalle', 'REP pendiente para pago de factura ' || f.numero || ' (registrado ' ||
                 EXTRACT(DAY FROM (now() - p.created_at))::int || ' día(s))',
      'documentos_faltantes', '[]'::jsonb,
      'monto_mxn', (p.monto_aplicado_factura * CASE
        WHEN p.moneda::text = 'MXN' THEN 1
    ...
    WHERE p.organization_id = p_organization_id
      AND p.deleted_at IS NULL
      AND p.estado_rep = 'Pendiente'
      AND p.uuid_rep IS NULL
      AND p.created_at < now() - INTERVAL '72 hours'
  ),
  hall_factura_cancel_sin_sust AS (
    -- Factura cancelada con motivo SAT 01 (sustitución) pero sin folio sustituto.
    SELECT jsonb_build_object(
      'embarque_id', e.id, 'expediente', e.expediente,
      'cliente_nombre', e.cliente_nombre, 'modo', e.modo::text, 'estado', e.estado::text, 'eta', e.eta,
