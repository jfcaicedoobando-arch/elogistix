-- ============================================================
-- BL-17(b) · calcular_comision_pago: la nota "Tipos de cambio del embarque
-- incompletos" exigía TC USD *y* EUR > 0 aunque el embarque no tuviera
-- ningún concepto en esa moneda → falsos positivos de "incompleto" en
-- embarques USD-only (ruido que erosiona la señal usada por BL-09).
--
-- Base: versión vigente creada por la migración 20260825000000 (BL-01).
-- Cuerpo VERBATIM salvo:
--   · se calculan los TCs realmente requeridos (monedas de los conceptos
--     de venta y costo del embarque) en `v_req_usd` / `v_req_eur`;
--   · la nota sólo se levanta cuando falta un TC requerido.
-- Misma firma, volatilidad, SECURITY DEFINER, search_path y grants.
-- ============================================================
