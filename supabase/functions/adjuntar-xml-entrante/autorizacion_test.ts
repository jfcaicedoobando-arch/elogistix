...
Deno.test("acepta sólo rutas dentro del prefijo canónico del documento", () => {
  assertEquals(pathPerteneceAlDocumento(`${ORG}/${EMB}/abc123-factura.xml`, ORG, EMB), true);
});
...
Deno.test("rechaza rutas de otra organización o de otro embarque", () => {
  assertEquals(pathPerteneceAlDocumento(`${OTRA_ORG}/${EMB}/a.xml`, ORG, EMB), false);
  assertEquals(pathPerteneceAlDocumento(`${ORG}/44444444-4444-4444-4444-444444444444/a.xml`, ORG, EMB), false);
  // Prefijo "parecido" que sólo comparte inicio de cadena.
  assertEquals(pathPerteneceAlDocumento(`${ORG}/${EMB}extra/a.xml`, ORG, EMB), false);
});

Deno.test("documento inexistente y documento de otra org dan el MISMO motivo (sin oráculo)", () => {
  const a = validarDocumento({ documento: null, orgActor: ORG, xmlPath: `${ORG}/${EMB}/a.xml` });
  const b = validarDocumento({ documento: doc, orgActor: OTRA_ORG, xmlPath: `${ORG}/${EMB}/a.xml` });
...
Deno.test("caso feliz: documento propio, por capturar y ruta canónica", () => {
  assertEquals(
    validarDocumento({ documento: doc, orgActor: ORG, xmlPath: `${ORG}/${EMB}/hash-a.xml` }),
    { ok: true },
  );
});
