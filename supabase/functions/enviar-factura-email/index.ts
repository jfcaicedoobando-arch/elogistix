 ...
 *
 * Homólogo a `enviar-cotizacion-email`, pero:
 *  - No necesita fase `prepare/upload` desde el cliente: el PDF y XML ya
 *    existen en FacturApi (una vez timbrada). La función los descarga con la
 *    API key de la organización, los sube al bucket privado `facturas-pdf`
 *    y firma URLs a 30 días.
 *  - Encola un correo por destinatario en la plantilla `factura-enviada`.
 *  - Registra el envío en `factura_envios` + bitácora.
 */
...
async function loadFactura(admin: ReturnType<typeof createClient>, id: string, userId: string) {
...
async function fetchFacturapiFile(apiKey: string, facturapiId: string, tipo: 'pdf' | 'xml'): Promise<Uint8Array> {
...
async function signUrl(admin: ReturnType<typeof createClient>, path: string): Promise<string> {
...
function parseBody(body: Record<string, unknown>): SendParsed {
