...
import { resolverGruposTrasladoDr, trasladoDesdeEncabezado } from "./trasladoDr.ts";

Deno.test("un renglón sin tipo_iva ya no cae al 16%", () => {
  assertEquals(resolverGruposTrasladoDr([{ tasa_iva_aplicada: 0.16 }]), "indeterminado");
  assertEquals(resolverGruposTrasladoDr([{ tipo_iva: null, tasa_iva_aplicada: null }]), "indeterminado");
  assertEquals(resolverGruposTrasladoDr([{ tipo_iva: "gravado_11", tasa_iva_aplicada: 0.11 }]), "indeterminado");
});

Deno.test("tipo y tasa contradictorios son indeterminados", () => {
  assertEquals(
    resolverGruposTrasladoDr([{ tipo_iva: "gravado_16", tasa_iva_aplicada: 0.08 }]),
    "indeterminado",
  );
  assertEquals(
    resolverGruposTrasladoDr([{ tipo_iva: "tasa_0", tasa_iva_aplicada: 0.16 }]),
    "indeterminado",
  );
});

Deno.test("tratamientos válidos siguen resolviendo su tasa canónica", () => {
  assertEquals(resolverGruposTrasladoDr([{ tipo_iva: "gravado_16" }]), [{ tasa: 0.16, factor: "Tasa", importe: 0 }]);
  assertEquals(resolverGruposTrasladoDr([{ tipo_iva: "gravado_8", tasa_iva_aplicada: 0.08 }]), [
    { tasa: 0.08, factor: "Tasa", importe: 0 },
  ]);
