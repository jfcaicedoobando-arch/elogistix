...
BEGIN
  ...
  v_moneda := COALESCE(v_old.moneda::text, 'MXN');
  ...
  ELSIF p_paso = 3 THEN
    ...
    ELSE
      v_crea := jsonb_build_array(jsonb_build_object(
        ...
        'monto', COALESCE(v_new.total, 0), 'moneda', COALESCE(v_new.moneda::text, v_moneda)));
    ...
    v_saldos := jsonb_build_array(
      jsonb_build_object('concepto', format('Factura %s', COALESCE(v_new.numero, 'nueva')),
        ...
        'moneda', COALESCE(v_new.moneda::text, v_moneda),
  ...
  ELSIF p_paso = 5 THEN
    IF v_pago.id IS NOT NULL AND v_c.factura_nueva_id IS NOT NULL THEN
      ...
      v_crea := jsonb_build_array(jsonb_build_object(
        ...
        'monto', v_aplicado, 'moneda', v_pago.moneda::text));
    ELSE
      v_bloqueos := v_bloqueos || to_jsonb('LC_REFACT_SIN_FACTURA_NUEVA'::text);
    END IF;

    IF v_pago.moneda IS NOT NULL AND v_new.id IS NOT NULL
       AND v_pago.moneda::text <> COALESCE(v_new.moneda::text, v_pago.moneda::text) THEN
