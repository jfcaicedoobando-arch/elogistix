...
- Cobertura focalizada: `tabCostosSinTipoCambio.test.tsx`, `reconciliacion/__tests__/reconciliacionPendiente.test.ts`, `services/__tests__/agruparRealesFacturados.moneda.test.ts`, más regresiones en `sumarFacturas.test.ts` y `useCotizacionKeyboardShortcuts.test.tsx`.
...
- **fix(H6)**: `public._cotizaciones_bloquear_auto_aceptacion()` se reemite con privilegios explícitos (`REVOKE ALL … FROM PUBLIC, anon` + `GRANT EXECUTE … TO service_role`); el cuerpo (guard acotado a la aceptación real) no cambia. Espejo: `supabase/schema/cotizaciones/_cotizaciones_bloquear_auto_aceptacion.sql`.
- **fix(tesorería · N6)**: `fetchLiquidacionesPendientes` ya no pide una sola página con `.limit(CAP_LISTA)`: lee TODAS las páginas con `leerTodasLasPaginas` (`tesoreria.liquidacionesPendientes`, lote `CAP_POSTGREST`, tope duro fail-visible). Con más de 500 liquidaciones pendientes el flujo proyectado subestimaba egresos en silencio. Cobertura: `flujoProyectado.truncado.test.ts`.
- **fix(CxC · N7)**: un REP cancelado ante el SAT ya NO consume saldo en `public.assert_factura_viva_para_pago` ni en `public.registrar_pago_cliente_lote`; ambos usan el predicado canónico `public.pago_rep_anulado(estado_rep)` (igual que `_saldo_factura_calc` y `cartera_pendiente`). Antes, tras cancelar un REP la UI mostraba saldo y la BD rechazaba el cobro de reemplazo como sobrepago. Locks y tolerancia (0.005) sin cambios. Cobertura: `supabase/tests/n7_rep_cancelado_no_consume_saldo.sql`.
- **fix(CxP · N8)**: `public.registrar_pago_proveedor_atomico` valida (con `FOR UPDATE`) que la cuenta bancaria exista, esté **activa** y sea de la **misma organización** de la factura antes de insertar el pago (`LC_PAGO_CUENTA_INEXISTENTE`, `LC_PAGO_CUENTA_OTRA_ORG`, `LC_PAGO_CUENTA_INACTIVA`); `public._asegurar_movimiento_pago_proveedor` repite la validación como defensa en profundidad (`LC_MOVIMIENTO_CUENTA_OTRA_ORG`, `LC_MOVIMIENTO_CUENTA_INACTIVA`). El flujo válido y los traspasos no cambian. Cobertura: `supabase/tests/n8_pago_proveedor_cuenta_valida.sql`.
- **fix(CxC · N9)**: el saldo de la factura en el detalle y en el diálogo de cobro viene del canon servidor `public.saldo_factura` (notas de crédito ya convertidas a la moneda de la factura). `calcularSaldoFactura` acepta `saldoServidor` y deriva las NC convertidas de la identidad del canon: la conversión NO se duplica en TypeScript. Se conserva la lista de notas para la vista de documentos. Cobertura: `saldoFactura.saldoServidor.test.ts`.
- **fix(cobranza · N10)**: `public.cartera_pendiente()` mantiene su `LIMIT 500`, pero se agrega `public.cartera_pendiente_total()` (RLS nativo) y la pantalla de Cobranza muestra un aviso explícito de vista incompleta con el total real en vez de presentar el corte como la cartera completa (antes `assertNotTruncated` dejaba la pantalla en error, sin datos). Cobertura: `carteraPendienteTruncado.test.ts`. Espejo: `supabase/schema/facturacion/cartera_pendiente_total.sql`.
...
- **fix(finanzas/tesorería)**: lote de integridad N1–N5 (sin publicar, sin pantallas nuevas, sin relajar RLS/guards).
  - **N1 saldo CxP con NC en otra moneda**: el listado de CxP (`fetchFacturasCxP`, `fetchFacturaProveedor`) y la bandeja de pagos programados (`fetchPagosProgramables`) ya no suman `proveedor_notas_credito.monto` en crudo: leen el saldo canónico del servidor desde `public.v_proveedor_facturas_saldo` (nuevo servicio `src/features/cxp/services/saldosProveedorFactura.ts`), que convierte pagos y NC con `monto_pago_en_moneda_factura`. `mapJoinedRow` acepta el saldo servidor y sólo clampa a 0; el select programable ya no arrastra embeds de pagos/NC. No se duplica conversión financiera en TypeScript.
  - **N2 estado de cuenta de proveedor**: `public.proveedor_estado_cuenta` agrega el CTE `nc_por_factura` (NC vivas `Aplicada`, convertidas con `monto_pago_en_moneda_factura`; sin TC quedan fuera) y suma `pagado + nc_aplicada` ANTES de prorratear hacia `conceptos_costo`. El trato de los pagos no cambia. Espejo: `supabase/schema/proveedores/proveedor_estado_cuenta.sql`.
  - **N3 fecha del movimiento manual**: nuevo trigger `trg_bbva_guard_movimiento_manual` / `public._bbva_guard_movimiento_manual()` para `hash_dedupe LIKE 'manual-%'` en INSERT y cambios de `fecha`: fecha requerida, no mayor a `public.fecha_negocio_mx()` y no anterior a `cuentas_bancarias.fecha_saldo_inicial` (`LC_MOVIMIENTO_MANUAL_FECHA_REQUERIDA` / `_FUTURA` / `_ANTES_CORTE`). Importaciones históricas y movimientos de sistema no se ven afectados. En la UI, `validarMovimientoManual` recibe límites (`hoyNegocio`, `fechaCorte`) y el `DatePickerMx` del diálogo acota `min`/`max`.
  - **N4 idempotencia del movimiento manual**: la llave se genera al ABRIR el diálogo (`claveIdempotencia`) y viaja al servicio, que la usa como `hash_dedupe` estable; ante `23505` del índice único de dedupe vivo se confirma el movimiento existente y se considera éxito, sin crear otro ni duplicar bitácora.
  - **N5 sentido bancario al conciliar**: `assert_movimiento_pago_consistente` exige dirección — `pago_factura_id`/`pago_factura_lote_id` sólo con abono > 0 y cargo = 0 (`LC_MOVIMIENTO_SENTIDO_COBRO`); `pago_proveedor_id`/`pago_proveedor_lote_id`/`anticipo_proveedor_id` sólo con cargo > 0 y abono = 0 (`LC_MOVIMIENTO_SENTIDO_PAGO`). Los traspasos y vínculos de sistema se conservan. `conciliarConPago` valida el sentido antes del UPDATE con mensaje claro.
  - Mensajes canónicos nuevos en `src/lib/errors/lcCodeMessages.tesoreria.ts` (5 códigos LC_*); textos técnicos intactos en SQL.
  - Espejos/canon: `supabase/schema/tesoreria/_bbva_guard_movimiento_manual.sql`, `supabase/schema/tesoreria/assert_movimiento_pago_consistente.sql`, `supabase/schema/proveedores/proveedor_estado_cuenta.sql`.
  - Cobertura nueva: `supabase/tests/n3_n5_movimientos_bancarios.sql` (registrada en `_guards_manifest.txt`), `src/lib/__tests__/n2-proveedor-estado-cuenta-nc.test.ts`, `src/features/tesoreria/domain/__tests__/movimientoManualFechas.test.ts`, `src/features/tesoreria/services/__tests__/conciliacionManualIdempotente.test.ts`, `src/features/cxp/services/__tests__/saldoServidorCxP.test.ts`; actualizadas `conciliacion.test.ts` y `pagosProgramados.test.ts` al canon vigente.
...
- **fix(seguridad/H6)**: segundo hallazgo de CI #4062 — la migración `20260914175016_…` reemitía `public.ejecutar_pago_programado(uuid, uuid, date, numeric, text, text, uuid)` (cambio M3 de fecha de negocio MX) sin reafirmar sus privilegios. Se añaden al final de esa migración, y en una migración de refuerzo aplicada, `REVOKE ALL … FROM PUBLIC, anon` y `GRANT EXECUTE … TO authenticated` — exactamente los roles que ya tenía antes de reemitirse (sin expandir acceso). Espejo actualizado: `supabase/schema/tesoreria/ejecutar_pago_programado.sql`. El cambio M3 de fecha se conserva íntegro.
...
- El candado de cierre permite registrar cobros y pagos de lo ya comprometido en embarques Cerrados; borrarlos o cambiar importes sigue bloqueado.
...
- **test(cxp)**: nuevo guard `supabase/tests/cxp_sobrecosto_misma_moneda.sql` (misma moneda sin exceso, exceso real >5% en la misma moneda, cruce de monedas).
...
- `ensureFreshSession.ts`: si otra pestaña o la renovación automática ya usó el refresh token, espera brevemente la sesión rotada y sólo acepta el nuevo JWT; nunca reenvía el token anterior por conservar una fecha local futura.
- Regresiones focalizadas para comprobar la renovación preventiva, la carrera entre pestañas y que no se llame a `parse-invoice-pdf` sin una sesión renovada. Sin cambios de SQL, RLS, permisos ni datos. CI y RLS completos quedan para GitHub Actions.
...
- `ensureFreshSession.ts`: cuando `parse-invoice-pdf` ya rechazó el JWT y la renovación forzada falla con `Refresh Token Not Found`, ya no se reutiliza el mismo token por tener una fecha local todavía vigente. El respaldo de una renovación preventiva también conserva el margen de seguridad de 60 segundos, cerrando la ventana de carrera con la rotación automática. La carga termina con el mensaje de sesión requerido en vez de repetir el 401 técnico y provocar una pantalla en blanco.
