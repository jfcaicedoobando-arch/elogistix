...
| Bucket | # | Significado |
|--------|--:|-------------|
| OK | 151 | Filtro por PK/FK, count-only, o ya pagina. |
| CATALOG | 23 | Tabla de catálogo acotado (allowlist). |
| RISK | 0 | Sin paginar y sin filtro acotante — candidato a .range/.limit. |
...
- `src/services/crm/forecast.ts:45` — `crm_motivos_perdida`
- `src/services/crm/forecast.ts:46` — `crm_etapas_pipeline`
- `src/services/crm/leaderboard.ts:37` — `crm_etapas_pipeline`
...
- `src/services/crm/forecast.ts` — 4
...
- `src/services/crm/leaderboard.ts` — 2
...
- `src/services/auditoria/snapshots.ts` — 1
...
- `src/services/facturas/index.ts` — 1
