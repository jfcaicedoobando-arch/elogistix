...
import { useCrearActividad, useOportunidades, type CrmEntidadTipo } from "@/hooks/crm";
...
export default function QuickCreateActividadPopover({ onCreated, onMore, onClose }: Props) {
  ...
  const submit = async () => {
    const a = asunto.trim();
    if (!a) return toast.error("Asunto requerido");
    if (!entidadId) return toast.error("Selecciona una oportunidad");
    try {
