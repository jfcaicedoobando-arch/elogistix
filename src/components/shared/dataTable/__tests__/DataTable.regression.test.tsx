...
describe("sortingFns — colación es-MX y nulls al final", () => {
  ...
  it("colación es-MX (acentos y mayúsculas insensibles)", () => {
    ...
    expect(fn(mkS("árbol"), mkS("banana"), "v", false)).toBeLessThan(0);
    expect(fn(mkS("ARBOL"), mkS("arbol"), "v", false)).toBe(0);
  });

  it("sortByString manda null/undefined al final", () => {
    const fn = sortByString<SR>((r) => r.v);
    expect(fn(mkS(null), mkS("a"), "v", false)).toBeGreaterThan(0);
    expect(fn(mkS("a"), mkS(null), "v", false)).toBeLessThan(0);
    expect(fn(mkS(null), mkS(null), "v", false)).toBe(0);
  });

  it("sortByNumber respeta nulls al final", () => {
    const fn = sortByNumber<NR>((r) => r.n);
    expect(fn(mkN(10), mkN(null), "n", false)).toBeLessThan(0);
    expect(fn(mkN(null), mkN(10), "n", false)).toBeGreaterThan(0);
    expect(fn(mkN(1), mkN(2), "n", false)).toBeLessThan(0);
  });

  it("sortByDate compara timestamps y trata strings inválidos como nulls", () => {
    const fn = sortByDate<DR>((r) => r.d);
    expect(fn(mkD("2024-01-01"), mkD("2025-01-01"), "d", false)).toBeLessThan(0);
    expect(fn(mkD("no-es-fecha"), mkD("2024-01-01"), "d", false)).toBeGreaterThan(0);
  });
});
