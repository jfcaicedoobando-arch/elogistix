...
import { useAllTiposContenedor, useAdminTiposContenedor } from "@/hooks/catalogos/useTiposContenedor";
...
export default function TabTiposContenedor() {
  ...
  const columns: DataTableColumn<TipoContenedor>[] = [
    ...
    },
    {
      ...
      render: (t) => (
        <Button variant="ghost" size="icon" className="h-8 w-8 text-destructive hover:text-destructive" onClick={() => eliminarTipo.mutate(t.id)}>
