...
export default function TabNavieras() {
  ...
  const columns: DataTableColumn<Naviera>[] = [
    ...
    },
    {
      ...
      render: (n) => (
        <Button variant="ghost" size="icon" className="h-8 w-8 text-destructive hover:text-destructive" onClick={() => eliminarNaviera.mutate(n.id)} aria-label={`Eliminar naviera ${n.nombre}`}>
