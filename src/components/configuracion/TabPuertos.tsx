...
export default function TabPuertos() {
  ...
  const columns: DataTableColumn<Puerto>[] = [
    ...
    },
    {
      ...
      render: (p) => (
        <Button variant="ghost" size="icon" className="h-8 w-8 text-destructive hover:text-destructive" onClick={() => eliminarPuerto.mutate(p.id)} aria-label={`Eliminar puerto ${p.nombre}`}>
