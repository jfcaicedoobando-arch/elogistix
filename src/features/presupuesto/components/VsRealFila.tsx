...
export function VsRealFila({ fila, striped }: Props) {
  ...
  return (
    <DetailTableRow className={cn(striped && "bg-muted/20")}>
      ...
      <TableCell className="text-right tabular-nums">{formatCurrency(fila.presupuesto_mxn, "MXN")}</TableCell>
      <TableCell className="text-right tabular-nums">{formatCurrency(fila.real_mxn, "MXN")}</TableCell>
      <TableCell className={cn("text-right tabular-nums font-medium", varClass)}>
