  ...
  return (
    <div className="flex items-start justify-between gap-2 pt-1">
      <div className="flex items-center gap-2 min-w-0">
        <div
          className={cn(
            "size-8 rounded-full flex items-center justify-center font-semibold shrink-0 text-body",
...
export function CostoTotalBlock({
  ...
  return (
    <div className={cn("rounded-md p-3 -mx-1", esGanador ? "bg-success/10" : "bg-muted/50")}>
      ...
      {!esGanador && delta > 0 && (
        <div className="text-body-sm text-destructive font-medium mt-1 text-right">
          +{usd(delta)} vs #1
        </div>
      )}
    </div>
...
export function FechaVigencia({ vigenteHasta, vencePronto }: { vigenteHasta: string; vencePronto: boolean }) {
  return (
    <div className="flex items-center gap-1.5 text-body-sm">
      ...
      {vencePronto && (
        <Badge variant="outline" className="bg-destructive/15 text-destructive border-destructive/30 text-label ml-auto">
          Vence pronto
        </Badge>
      )}
    </div>
...
export function EtiquetasList({ etiquetas }: { etiquetas: string[] }) {
  ...
  return (
    <div className="flex flex-wrap gap-1">
      {etiquetas.map((e) => (
        <Badge key={e} variant="secondary" className="text-label uppercase tracking-wide font-semibold">
          {e}
        </Badge>
      ))}
    </div>
