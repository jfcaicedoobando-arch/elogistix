...
export function TarifaForm({ open, onOpenChange, initial }: Props) {
  ...
  const [form, setForm] = useState<TarifaInput>({
    ...
    naviera_id: initial?.naviera_id ?? "",
    ruta_id: initial?.ruta_id ?? "",
    tipo_contenedor_id: initial?.tipo_contenedor_id ?? "",
    flete_base: initial?.flete_base ?? 0,
    dias_libres_demoras: initial?.dias_libres_demoras ?? 7,
    vigente_desde: initial?.vigente_desde ?? todayISO(),
    vigente_hasta: initial?.vigente_hasta ?? plusDays(30),
    transit_time_dias: initial?.transit_time_dias ?? null,
    notas: initial?.notas ?? "",
    recargos: initial?.recargos ?? [],
  });
  ...
  }, [form.flete_base, form.recargos]);

  const valido =
    form.agente_id && form.naviera_id && form.ruta_id && form.tipo_contenedor_id &&
    form.flete_base > 0 && form.vigente_desde <= form.vigente_hasta;
