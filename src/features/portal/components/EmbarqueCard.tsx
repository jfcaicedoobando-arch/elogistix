...
function EmbarqueCardInner({ e }: { e: EmbarqueCardData }) {
  ...
  return (
    <article {...rowNav} className={cn(rowNav.className, "block")}>
      <Card className={`border-l-4 ${getEstadoBorderColor(estadoVisual)} hover:shadow-raised hover:scale-[1.005] transition-[box-shadow,transform] duration-200 group`}>
          ...
          <div className="flex items-center justify-between gap-2">
            <div className="flex items-center gap-3 min-w-0">
              ...
              <p className="font-semibold text-body truncate font-mono tabular-nums">
                {labelExpediente(e.expediente, e.id, e.estado)}{e.contenedor ? ` — ${e.contenedor}` : ""}
              </p>
            </div>
            <StatusBadge
              domain="embarque"
