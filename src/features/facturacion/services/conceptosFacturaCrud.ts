 ...
 *
 * Regla de negocio: `total` en `conceptos_factura` = cantidad * precio_unitario
 * (subtotal por renglón, sin IVA). El IVA se suma agregando `tasa_iva` por
 * renglón (default 0.16) al recalcular totales de la factura.
 */
...
export interface ConceptoFacturaRow extends ConceptoFacturaInput {
  id: string;
  factura_id: string;
  moneda: string;
  total: number;
...
function normalizarLinea(input: ConceptoFacturaInput) {
  ...
  return {
    ...
    clave_unidad: input.clave_unidad?.trim() || null,
    unidad: input.unidad?.trim() || null,
    tasa_iva: input.tasa_iva == null ? 0.16 : Number(input.tasa_iva),
  };
}
...
export async function recalcularTotalesFactura(facturaId: string): Promise<void> {
  const { data, error } = await supabase
    .from("conceptos_factura")
    .select("cantidad, precio_unitario, tasa_iva")
  ...
  let iva = 0;
  for (const c of data ?? []) {
    const linea = Number(c.cantidad) * Number(c.precio_unitario);
    subtotal += linea;
    iva += linea * (c.tasa_iva == null ? 0.16 : Number(c.tasa_iva));
  }
...
export async function fetchConceptosFactura(facturaId: string): Promise<ConceptoFacturaRow[]> {
  const { data, error } = await supabase
    .from("conceptos_factura")
    .select("id, factura_id, descripcion, cantidad, precio_unitario, clave_sat, clave_unidad, unidad, tasa_iva, total, moneda")
    .eq("factura_id", facturaId)
    .order("created_at", { ascending: true });
  if (error) throw error;
  return (data ?? []) as unknown as ConceptoFacturaRow[];
}
