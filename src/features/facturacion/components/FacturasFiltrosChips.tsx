/**
 * Chips de filtros activos para el listado de Facturas.
 * Mismo patrón que `ProformasFiltrosChips`.
 */
...
export interface FacturasFiltrosChipsProps {
  filtroEstado: string;
...
export function FacturasFiltrosChips(props: FacturasFiltrosChipsProps) {
  const chips: ChipDef[] = [];

  if (props.filtroEstado && props.filtroEstado !== "todos") {
    chips.push({
      key: "estado",
      label: "Estado",
      value: props.filtroEstado,
      onRemove: () => props.onFiltroEstadoChange("todos"),
    });
  }
