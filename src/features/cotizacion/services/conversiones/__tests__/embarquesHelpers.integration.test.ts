...
describe("Integración Cotización → Embarque (helpers puros)", () => {
  ...
  describe("[integration] parsearVentasJsonb", () => {
    ...
    it("(v13.66.13) con hijos: replica unidad_medida='Contenedor' multiplicando monto por hijo", () => {
      const hijos = [{ id: "h1" }, { id: "h2" }, { id: "h3" }] as Pick<Tables<"embarque_contenedores">, "id">[];
      const out = parsearVentasJsonb(
        [{ descripcion: "Flete", unidad_medida: "Contenedor", cantidad: 1, precio_unitario: 2300, total: 2300 }],
        "emb-1",
        hijos,
      );
      ...
      expect(totalSuma).toBe(6900); // 3 × 2300, monto multiplicado
    });

    it("(v13.66.13) con hijos: unidad_medida='BL' se inserta UNA vez sin contenedor_id", () => {
    ...
    it("(v13.66.13) con hijos: cantidad=1 también replica multiplicando (sin fallback)", () => {
      const hijos = [{ id: "h1" }, { id: "h2" }, { id: "h3" }] as Pick<Tables<"embarque_contenedores">, "id">[];
      const out = parsearVentasJsonb(
        [{ descripcion: "Servicio por contenedor", unidad_medida: "Contenedor", cantidad: 1, precio_unitario: 500, total: 500 }],
        "emb-1",
        hijos,
      );
      expect(out).toHaveLength(3);
      expect(out.map((r) => r.contenedor_id)).toEqual(["h1", "h2", "h3"]);
      expect(out.every((r) => r.total === 500)).toBe(true);
    });

    it("(v13.66.13) con hijos: cantidad>1 se preserva en cada hijo (no se divide)", () => {
      const hijos = [{ id: "h1" }, { id: "h2" }, { id: "h3" }] as Pick<Tables<"embarque_contenedores">, "id">[];
      const out = parsearVentasJsonb(
        [{ descripcion: "Lote", unidad_medida: "Contenedor", cantidad: 2, precio_unitario: 100, total: 200 }],
        "emb-1",
        hijos,
      );
      expect(out).toHaveLength(3);
      expect(out.map((r) => r.cantidad)).toEqual([2, 2, 2]);
      expect(out.every((r) => r.total === 200)).toBe(true);
      const totalSuma = out.reduce((s, r) => s + Number(r.total), 0);
      expect(totalSuma).toBe(600); // 3 × 200
    });
