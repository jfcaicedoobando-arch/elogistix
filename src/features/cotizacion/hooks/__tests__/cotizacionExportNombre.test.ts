...
describe("nombreMostradoCotizacion", () => {
  it("usa prospecto_empresa en prospectos", () => {
    expect(nombreMostradoCotizacion({
      cliente_nombre: null, es_prospecto: true, prospecto_empresa: "Aceros del Norte",
    })).toBe("Aceros del Norte");
  });

  it("usa cliente_nombre en cotizaciones de cliente", () => {
    expect(nombreMostradoCotizacion({
      cliente_nombre: "Cliente SA", es_prospecto: false, prospecto_empresa: null,
    })).toBe("Cliente SA");
  });

  it("no rompe cuando faltan ambos", () => {
    expect(nombreMostradoCotizacion({ cliente_nombre: null })).toBe("");
  });
});
