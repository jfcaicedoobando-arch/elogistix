...
function Dato({ etiqueta, valor }: { etiqueta: string; valor: string }) {
  return (
    <div className="space-y-0.5">
      <p className="text-body-sm text-muted-foreground">{etiqueta}</p>
      <p className="truncate text-body font-medium">{valor}</p>
    </div>
  );
}
  ...
  return (
    <div className="space-y-3 rounded-md border bg-muted/40 p-3">
      <div className="flex items-center gap-2">
        ...
        <p className="text-body font-medium">Datos leídos del CFDI</p>
      </div>
      <div className="grid gap-3 sm:grid-cols-2">
        ...
        <p className="sm:col-span-2 truncate text-body-sm text-muted-foreground">UUID: {meta.uuid}</p>
      </div>
    </div>
  );
}
