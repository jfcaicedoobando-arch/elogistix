...
function isBusinessRuleViolation(event: Sentry.ErrorEvent, exc: unknown): boolean {
...
function isNetworkConnectivityNoise(event: Sentry.ErrorEvent, exc: unknown): boolean {
  const msg =
    (exc as { message?: unknown } | undefined)?.message ??
    event.exception?.values?.[0]?.value ??
    event.message;
 ...
 */
function isEmptySerializedRejection(event: Sentry.ErrorEvent): boolean {
